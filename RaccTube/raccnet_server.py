    ...
    <div style=${{display:'flex',gap:12,paddingTop:12}}>
      ...
      <div style=${{flex:1,minWidth:0}} onClick=${function(){if(!blocked)props.onWatch(post);}}>
        ...
        <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:2}}>
          <div style=${{fontSize:13,color:'#aaa'}}>${blocked?'0 likes · 0':(fmt(post.likeCount||0)+' likes'+(vcDislikeCount>0?' · '+fmt(vcDislikeCount)+' dislikes':'')+' · '+ago(post.indexedAt))}</div>
...
function FriendsFeed(props) {
  ...
  const [subTab,   setSubTab]   = useState(function(){return loadVideoTab();});
  const [loading,  setLoading]  = useState(false);
  ...
  useEffect(function(){
    ...
    async function load(){
      ...
      try{
        ...
        for(var i=0;i<convos.length&&!cancelled;i++){
          ...
          for(var j=0;j<msgs.length;j++){
            ...
            if(et==='app.bsky.embed.record'||et==='app.bsky.embed.record#view'){
              ...
              // Check for Bluesky list URIs in plain text (shared playlists)
              var listMatch2=m.text&&m.text.match(/at:\/\/(did:[^/\s]+)\/app\.bsky\.graph\.list\/([a-zA-Z0-9]+)/);
              // Check for Bluesky profile URLs
              var profileMatch=m.text&&m.text.match(/bsky\.app\/profile\/([a-zA-Z0-9._:-]+)/);
              if(listMatch2){
              ...
              } else if(profileMatch&&!m.text.match(/bsky\.app\/profile\/[^/\s]+\/post\//)){
                // Profile URL (not a post URL) → shared channel card
        ...
        // Hydrate channel items from bsky.app profile links
        var chanRaw=found.filter(function(x){return x._type==='channel'&&x.handle;});
        var hydratedChans=[];
        for(var ci=0;ci<chanRaw.length&&!cancelled;ci++){
          try{
            var chR=await api(PUB_PROXY+'/app.bsky.actor.getProfile?actor='+encodeURIComponent(chanRaw[ci].handle));
            if(chR.ok){var chD=await chR.json();if(chD&&chD.did)hydratedChans.push({sender:chanRaw[ci].sender,profile:chD,msgText:chanRaw[ci].msgText,sentAt:chanRaw[ci].sentAt});}
          }catch(e5){}
        }
  ...
  return html`<div style=${{padding:24}}>
    <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,
      ...
      <div style=${{display:'flex',gap:4,alignItems:'center'}}>
        ...
        <button style=${tabSt2(subTab==='Videos')} onClick=${function(){setSubTab('Videos');saveVideoTab('Videos');}}>Videos</button>
        <button style=${tabSt2(subTab==='Posts')}  onClick=${function(){setSubTab('Posts');saveVideoTab('Posts');}}>Posts</button>
      </div>
    </div>
    ${senders.length>0?html`<${FollowStrip} actors=${senders} onChannel=${props.onChannel}/>`:null}
    ${subTab==='Videos'?html`<div>
      ${playlistItems.length?html`<div style=${{marginBottom:28}}>
        <div style=${{color:'#aaa',fontSize:11,fontWeight:700,letterSpacing:1,textTransform:'uppercase',marginBottom:12}}>Shared Playlists</div>
        <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'24px 16px'}}>
          ${playlistItems.map(function(item,i){
            var pl=item.list;
            var sender=item.sender||{};
            var [plCardHov, setPlCardHov] = [false, function(){}]; // simple hover via onMouseEnter/Leave
            return html`<div key=${i}
              style=${{background:'#1a1a1a',border:'1px solid #2a2a2a',display:'flex',overflow:'hidden',cursor:'pointer',minHeight:100,transition:'border-color 0.15s'}}
              ...
              <div style=${{width:'42%',flexShrink:0,background:'#0a0a0a',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'}}>
                ${pl.avatar?html`<img src=${pl.avatar} style=${{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>`:
                  html`<svg width="44" height="44" viewBox="0 0 24 24" fill="#3f3f3f"><path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 9H9V9h10v2zm-4 4H9v-2h6v2zm4-8H9V5h10v2z"/></svg>`}
                <div style=${{position:'absolute',bottom:4,left:4,background:'rgba(0,0,0,0.75)',color:'#ccc',fontSize:10,padding:'2px 5px',display:'flex',alignItems:'center',gap:3}}>
                  ...
                  ${pl.listItemCount||0} ${(pl.listItemCount||0)===1?'video':'videos'}
                </div>
              </div>
              <div style=${{flex:1,minWidth:0,display:'flex',flexDirection:'column',justifyContent:'space-between',padding:'10px 10px 10px 12px'}}>
                <div>
                  <div class="clamp2" style=${{fontSize:14,fontWeight:600,color:'#f1f1f1',lineHeight:1.35}}>${pl.name||'Playlist'}</div>
                  <div style=${{fontSize:11,color:'#888',marginTop:4,display:'flex',alignItems:'center',gap:6}}>
                    <${Avatar} src=${sender.avatar} size=${14} onClick=${function(e){e.stopPropagation();props.onChannel&&props.onChannel(sender.handle);}}/>
                    <span style=${{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>${sender.displayName||sender.handle}</span>
                  </div>
                </div>
                <div style=${{fontSize:11,color:'#444',marginTop:4}}>${ago(item.sentAt)}</div>
              </div>
            </div>`;
          })}
        </div>
      </div>`:null}
      ${channelItems.length?html`<div style=${{marginBottom:28}}>
        <div style=${{color:'#aaa',fontSize:11,fontWeight:700,letterSpacing:1,textTransform:'uppercase',marginBottom:12}}>Shared Channels</div>
        <div style=${{display:'flex',flexDirection:'column',gap:0}}>
          ${channelItems.map(function(item,i){
            var p=item.profile;
            var sender=item.sender||{};
            return html`<div key=${i}
              style=${{display:'flex',alignItems:'center',gap:14,padding:'10px 0',borderBottom:'1px solid #1a1a1a',cursor:'pointer'}}
              onMouseEnter=${function(e){e.currentTarget.style.background='rgba(255,255,255,0.02)';}}
              onMouseLeave=${function(e){e.currentTarget.style.background='none';}}
              onClick=${function(){props.onChannel&&props.onChannel(p.handle);}}>
              <${Avatar} src=${p.avatar} size=${52}/>
              <div style=${{flex:1,minWidth:0}}>
                <div style=${{color:'#f1f1f1',fontSize:14,fontWeight:600}}>${p.displayName||p.handle}</div>
                <div style=${{color:'#aaa',fontSize:12,marginTop:2}}>@${p.handle} · ${fmt(p.followersCount||0)} subscribers</div>
                ${p.description?html`<div style=${{color:'#777',fontSize:12,marginTop:3,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:420}}>${p.description.slice(0,100)}${p.description.length>100?'…':''}</div>`:null}
              </div>
              <div style=${{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4,flexShrink:0}}>
                <div style=${{color:'#555',fontSize:11}}>from @${sender.handle||'friend'} · ${ago(item.sentAt)}</div>
                <button onClick=${function(e){e.stopPropagation();props.onChannel&&props.onChannel(p.handle);}}
                  style=${{background:'var(--accent-solid-dim)',border:'1px solid var(--accent)',color:'var(--accent)',padding:'4px 10px',fontSize:11,cursor:'pointer',borderRadius:0,fontWeight:600}}>View Channel</button>
              </div>
            </div>`;
          })}
        </div>
      ...
      <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'24px 16px'}}>
        ${sortedItems.length?sortedItems.map(function(item,i){
    ...
    async function load() {
      try {
        ...
        for (var bi=0; bi<batches.length; bi++) {
          ...
          // Stream results in real-time after each batch
          if (!cancelled) {
            var sortedVids = allVids.slice().sort(function(a,b){return (b.likeCount||0)-(a.likeCount||0);});
            setVids(sortedVids);
...
function SubsPage(props) {
  const [tab,         setTab]         = useState(function(){return loadVideoTab();});
...
function FeedPage(props) {
  ...
  useEffect(function(){
    restoreScrollPos('feedpage_'+( props.feedUri||''));
    return function(){ saveScrollPos('feedpage_'+(props.feedUri||'')); };
  },[props.feedUri]);
  ...
  return html`<div style=${{padding:24}}>
    ...
    ${tab==='Videos'?html`<${VideoGrid} videos=${props.videos} loading=${props.loading} session=${props.session} onWatch=${props.onWatch} onChannel=${props.onChannel} sortOrder=${sortOrder} setSortOrder=${setSortOrder} timeRange=${timeRange} setTimeRange=${setTimeRange} hideLiked=${hideLiked} setHideLiked=${setHideLiked} contentFilter=${contentFilter} setContentFilter=${setContentFilter} hideHistory=${hideHistory} setHideHistory=${setHideHistory} hasMoreFromAPI=${!!props.hasMoreFromAPI} onLoadMoreAPI=${props.onLoadMoreAPI||null}/>`:null}
...
async function sendPromoReply(sess, postUri, postCid) {
  ...
  try {
    var parts = postUri.replace('at://','').split('/');
    var repoDid = parts[0], rkey = parts[2];
    var replyRef = {
      root: {uri:postUri,cid:postCid},
      parent: {uri:postUri,cid:postCid}
    };
    await bskyCreate(sess, 'app.bsky.feed.post', {
      '$type': 'app.bsky.feed.post',
      text: 'This was uploaded on RaccNet, an open-source YouTube-like AT Protocol platform! Download the newest WIP release at https://github.com/RechoRaccoon/RaccNet',
      reply: replyRef,
      createdAt: new Date().toISOString()
    });
    ...
    <form onSubmit=${submit} style=${{display:'flex',gap:8,alignItems:'center'}}>
      ...
      <textarea value=${text} onInput=${function(e){setText(e.target.value);}}
        ...
        style=${{flex:1,background:'#1a1a1a',border:'1px solid #333',color:'#f1f1f1',
          ...
          fontFamily:"'Roboto',sans-serif",height:36,lineHeight:'22px',overflow:'hidden'}}
        onFocus=${function(e){e.target.style.borderColor='var(--accent)';}}
...
function WatchPage(props) {
  ...
  const embed  = post.embed;
  const sess   = props.session;
  const parts        = buildPartsList(props.thread);
  ...
  const [localThread, setLocalThread] = useState(props.thread);
  const partUris = new Set(parts.slice(1).map(function(p){return p.uri;}));
  const replies = ((localThread && localThread.replies) || []).filter(function(r){return r.post && !partUris.has(r.post.uri);}).slice(0,50);
  const postId  = post.uri.split('/').pop();

  useEffect(function(){ setLocalThread(props.thread); }, [props.thread]);
    ...
    if(authorDid) {
      ...
      (async function(){
        try{
          ...
          if(cancelled) return;
          // Fetch videos in batches of 10, stream results in real-time
  ...
  return html`<div style=${{display:'flex',gap:24,padding:24,maxWidth:1600,margin:'0 auto'}}>${showShare?html`<${ShareModal} post=${displayPost} session=${sess} onClose=${function(){setShowShare(false);}}/>`  :null}
    <div style=${{flex:1,minWidth:0}}>
      ...
      <div style=${{background:'#1a1a1a',border:'1px solid #272727',padding:'12px 16px',marginBottom:24}}>
        <div style=${{fontSize:13,color:'#f1f1f1',fontWeight:500,marginBottom:4}}>
          ${fmt(likeCount)} likes · ${fmt(dislikeCount)} dislikes · ${fmt((localThread&&localThread.post&&localThread.post.replyCount!=null?localThread.post.replyCount:post.replyCount)||0)} comments · ${fmt(repostCount)} reposts · ${ago(post.indexedAt)}
        </div>
      ...
      <h3 style=${{color:'#f1f1f1',fontSize:16,fontWeight:600,marginBottom:16}}>${fmt((localThread&&localThread.post&&localThread.post.replyCount!=null?localThread.post.replyCount:post.replyCount)||0)} Comments</h3>
    ...
    <div style=${{width:402,flexShrink:0}}>
      ...
      ${props.related.length===0?html`<div style=${{color:'#aaa',fontSize:14,marginBottom:24}}>No related videos.</div>`:
        html`<div style=${{overflowY:'hidden',maxHeight:330,transition:'max-height 0.2s'}}
          ...
          <${VideoListCompact} videos=${props.related} onWatch=${props.onWatch} onChannel=${props.onChannel}/>
        </div>`
      }
      ...
      ${subVids.length>0?html`
        <h3 style=${{color:'#f1f1f1',fontSize:15,fontWeight:600,marginBottom:4,marginTop:28,display:'flex',alignItems:'center',gap:8}}>
        ...
        <div style=${{overflowY:'hidden',maxHeight:330,transition:'max-height 0.2s'}}
          onMouseEnter=${function(e){e.currentTarget.style.overflowY='auto';}}
          onMouseLeave=${function(e){e.currentTarget.style.overflowY='hidden';}}>
          <${VideoListCompact} videos=${subVids} onWatch=${props.onWatch} onChannel=${props.onChannel}/>
        </div>
      `:null}
...
function CommentRow(props) {
  ...
  return html`<div style=${{marginLeft:depth>0?18:0,paddingLeft:depth>0?12:0,borderLeft:depth>0?'2px solid #2a2a2a':'none'}}>
    <div style=${{display:'flex',gap:10,padding:'10px 0',borderBottom:'1px solid #1a1a1a'}}>
...
function ChannelPage(props) {
  ...
  const [contentSub,  setContentSub]  = useState(function(){return loadVideoTab();});
  ...
  useEffect(function() {
    ...
    if (prevPropsDidRef.current && prevPropsDidRef.current !== incomingDid) {
      setProfileData(null);
      setTab(props.initialTab || 'Content');
      setContentSub(loadVideoTab());
      setShowEdit(false);
      setLikedLoaded(false); setLikedPosts([]); setLikedVids([]);
      setAllPosts([]); setLoadedDid(null);
      setReposts([]); setRepostsLoaded(false);
      setDmTabKey(function(k){return k+1;});
      setHasDMs(false);
      setChannelLists(null); setChannelPlaylists(null); setChannelStPacks(null);
      setPlaylistView(null); setPlaylistVids([]); setStarterPackView(null);
      setChannelFeeds(null);
    }
  ...
  async function openContentSub(s) {
    setContentSub(s); saveVideoTab(s);
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid var(--accent)'}}>
      <div style=${{display:'flex',alignItems:'center',gap:20,padding:'20px 0 20px'}}>
        ...
        <div style=${{flex:1,minWidth:0}}>
          ...
          </span> · ${props.videos.length} videos</div>
    ...
    <div style=${{padding:24}}>
      ${tab==='Content'&&contentSub==='Videos'?html`<${VideoGrid} videos=${props.videos} loading=${false} session=${props.session} onWatch=${props.onWatch} onChannel=${props.onChannel} sortOrder=${sortOrder} setSortOrder=${setSortOrder} timeRange=${timeRange} setTimeRange=${setTimeRange} hideLiked=${hideLiked} setHideLiked=${setHideLiked} contentFilter=${contentFilter} setContentFilter=${setContentFilter}/>`:null}
      ${tab==='Content'&&contentSub==='Posts'?html`<div>
        ...
        <${ChannelPostsFeed} posts=${allPosts} loading=${postsLoading} session=${props.session} onChannel=${props.onChannel} onWatch=${props.onWatch} stateKey=${'ch:'+(props.data&&props.data.handle||'')} sortOrder=${sortOrder} setSortOrder=${setSortOrder} timeRange=${timeRange} setTimeRange=${setTimeRange}/>
      </div>`:null}
      ${tab==='Reposts'?html`<div>${(function(){
        ...
        return html`<${ChannelPostsFeed} posts=${reposts} loading=${false} session=${props.session} onChannel=${props.onChannel} onWatch=${props.onWatch} stateKey=${'reposts:'+(props.data&&props.data.handle||'')} sortOrder=${sortOrder} setSortOrder=${setSortOrder} timeRange=${timeRange} setTimeRange=${setTimeRange}/>`;
      })()}</div>`:null}
      ...
      ${tab==='Feeds'?html`<div>
        ...
        ${(channelFeeds||[]).map(function(f,i){return html`<div key=${i} style=${{display:'flex',alignItems:'center',gap:14,padding:'12px 0',borderBottom:'1px solid #1a1a1a'}}>
          ${f.avatar?html`<img src=${f.avatar} style=${{width:44,height:44,objectFit:'cover',flexShrink:0,background:'#272727'}} onError=${function(e){e.target.style.display='none';}}/>`:
            html`<div style=${{width:44,height:44,background:'var(--accent-solid-dim)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--accent)',fontSize:18}}>≡</div>`}
          <div style=${{flex:1,minWidth:0}}>
            <div style=${{color:'#f1f1f1',fontWeight:500,fontSize:13}}>${f.displayName||f.uri.split('/').pop()}</div>
          ...
          <a href=${'https://bsky.app/profile/'+encodeURIComponent(d.handle)+'/feed/'+f.uri.split('/').pop()} target="_blank" rel="noopener noreferrer"
            style=${{padding:'5px 10px',border:'1px solid #3f3f3f',background:'none',color:'#f1f1f1',fontSize:12,cursor:'pointer',borderRadius:0,textDecoration:'none',flexShrink:0}}
            onMouseEnter=${function(e){e.currentTarget.style.borderColor='var(--accent)';e.currentTarget.style.color='var(--accent)';}}
...
function App() {
  ...
  const navHistoryRef = useRef([{page: loadLastPage()||'subs', scrollY:0}]);
  ...
  const [channelInitialTab, setChannelInitialTab] = useState(null);
  const [subFeedTarget, setSubFeedTarget] = useState(null); // {did, handle} for SubscribedFeedPage
  ...
  // Restore scroll position after page change (for navBack/navForward)
  useEffect(function(){
    if(restoreScrollRef.current !== null){
      var y = restoreScrollRef.current;
  ...
  function navTo(p) {
    ...
    // Save current scroll to last history entry before navigating away
    var hist = navHistoryRef.current;
    if(hist.length > 0) hist[hist.length-1].scrollY = window.scrollY;
    hist.push({page:p, scrollY:0});
  ...
  function navBack() {
    ...
    var current = hist.pop();
    current.scrollY = window.scrollY;
    navFutureRef.current.push(current);
    var prev = hist[hist.length-1];
    setPage(prev.page);
    if(prev.page!=='channel'&&prev.page!=='watch') saveLastPage(prev.page);
    restoreScrollRef.current = prev.scrollY || 0;
  }
  function navForward() {
    var fut = navFutureRef.current;
    ...
    hist.push(next);
    setPage(next.page);
    if(next.page!=='channel'&&next.page!=='watch') saveLastPage(next.page);
  ...
  const loadFeedVideos = useCallback(async function(feedUri, sess){
    ...
    function add(posts, d){
      (posts||[]).forEach(function(p){if(p&&isVid(p)&&!seen.has(p.uri)&&!(p.record&&p.record.reply)){videos.push(p);seen.add(p.uri);}});
    }
    try{
      const authOpts = sess?{headers:{Authorization:'Bearer '+sess.accessJwt}}:{};
      const endpoint = sess?AUTH_PROXY:PUB_PROXY;
      let r=null, authFailed=false, cursor=null;
      try { r = await api(endpoint+'/app.bsky.feed.getFeed?feed='+encodeURIComponent(feedUri)+'&limit=100',authOpts); } catch(e){ r=null; }
      if(r&&r.ok){ const d=await r.json(); add((d.feed||[]).map(function(i){var p=i.post;if(i.reason&&i.reason['$type']==='app.bsky.feed.defs#reasonRepost'&&i.reason.by)p=Object.assign({},p,{_repostedBy:i.reason.by});return p;})); cursor=d.cursor||null; }
      ...
      if(videos.length<10 && !authFailed){
        ...
        if(r2&&r2.ok){ const d=await r2.json(); add((d.feed||[]).map(function(i){var p=i.post;if(i.reason&&i.reason['$type']==='app.bsky.feed.defs#reasonRepost'&&i.reason.by)p=Object.assign({},p,{_repostedBy:i.reason.by});return p;})); if(!cursor) cursor=d.cursor||null; }
      }
  ...
  const loadMoreFeedVideos = useCallback(async function(){
    if (!feedCursor || !activeFeed || feedLoading) return;
    setFeedLoading(true);
    try {
      const authOpts = session?{headers:{Authorization:'Bearer '+session.accessJwt}}:{};
      const endpoint = session?AUTH_PROXY:PUB_PROXY;
      const r = await api(endpoint+'/app.bsky.feed.getFeed?feed='+encodeURIComponent(activeFeed)+'&limit=100&cursor='+encodeURIComponent(feedCursor), authOpts);
      if(r&&r.ok){
        const d = await r.json();
        const newVids = (d.feed||[]).map(function(i){
          var p=i.post;
          if(i.reason&&i.reason['$type']==='app.bsky.feed.defs#reasonRepost'&&i.reason.by) p=Object.assign({},p,{_repostedBy:i.reason.by});
          return p;
        }).filter(function(p){return p&&isVid(p)&&!(p.record&&p.record.reply);});
        setFeedVideos(function(prev){
          var seen=new Set(prev.map(function(p){return p.uri;}));
          return prev.concat(newVids.filter(function(p){return !seen.has(p.uri);}));
        });
        setFeedCursor(d.cursor||null);
      }
    } catch(e){ console.error('loadMoreFeedVideos:',e); }
    setFeedLoading(false);
  },[feedCursor, activeFeed, session, feedLoading]);
  ...
  const handleWatch = useCallback(async function(post){
    ...
    addToHistory(post);
    saveLastWatch(watchUri); saveLastPage('watch');
    setCurrentVideo(post); setThread(null); setRelated([]);
    var _hw = navHistoryRef.current;
    if(_hw.length>0) _hw[_hw.length-1].scrollY = window.scrollY;
    _hw.push({page:'watch', scrollY:0});
    if(_hw.length > 50) _hw.shift();
    navFutureRef.current = [];
      ...
      if(tR.ok){
        ...
        if(replyRef&&replyRef.root&&replyRef.root.uri&&replyRef.root.uri!==post.uri){
          try{
            ...
            if(rootR.ok){const rd=await rootR.json();threadToUse=rd.thread;}
          }catch(e){}
        }
        setThread(threadToUse);
      }
  ...
  const handleSubFeed = useCallback(function(did, handle) {
    setSubFeedTarget({did:did, handle:handle});
    var _nh = navHistoryRef.current;
    if(_nh.length>0) _nh[_nh.length-1].scrollY = window.scrollY;
    _nh.push({page:'subfeed', scrollY:0});
    if(_nh.length>50) _nh.shift();
    saveLastPage('subfeed');
    setPage('subfeed');
    window.scrollTo(0,0);
  }, []);

  // ── Channel ─────────────────────────────────────────────────────────────────
  const handleChannel = useCallback(async function(actor, opts){
    if(!actor) return;
    setChannelFromPage(pageRef.current);
    setChannelInitialTab((opts && opts.openTab) || null);
    saveLastChan(actor); saveLastPage('channel');
    setChannelData(null); setChannelVideos([]); setChannelLoading(true);
    var _hc = navHistoryRef.current;
    if(_hc.length>0) _hc[_hc.length-1].scrollY = window.scrollY;
    _hc.push({page:'channel', scrollY:0});
    if(_hc.length > 50) _hc.shift();
    navFutureRef.current = [];
  ...
  <div style=${{minHeight:'100vh',background:'#0f0f0f',color:'#f1f1f1'}}>
    ...
    <${Sidebar}
      ...
      onWatchlist=${function(){navTo('watchlist');}}
    ...
    <main style=${{marginLeft:mL,marginTop:56+uploadBarH,minHeight:'calc(100vh - 56px)',transition:'margin-left 0.15s ease, margin-top 0.15s ease'}}>
      ${page==='history'?html`<${HistoryPage} session=${session} onWatch=${handleWatch} onChannel=${handleChannel}/>`:null}
      ${page==='watchlist'?html`<${WatchlistPage} session=${session} onWatch=${handleWatch} onChannel=${handleChannel}/>`:null}
      ...
      ${page==='subfeed'&&subFeedTarget?html`<${SubscribedFeedPage} did=${subFeedTarget.did} handle=${subFeedTarget.handle} session=${session} onBack=${navBack} onWatch=${handleWatch} onChannel=${handleChannel}/>`:null}
      ${page==='search'?html`<${SearchPage} results=${searchResults} loading=${searchLoading} query=${searchQuery} session=${session} onWatch=${handleWatch} onChannel=${handleChannel} onFeedSelect=${handleFeedSelect} onSignIn=${function(){setShowLogin(true);}}/>`:null}
      ${page==='feed'?html`<${FeedPage} videos=${feedVideos} loading=${feedLoading} feedName=${currentFeedName} feedUri=${activeFeed} session=${session} onWatch=${handleWatch} onChannel=${handleChannel} hasMoreFromAPI=${!!feedCursor} onLoadMoreAPI=${loadMoreFeedVideos}/>`:null}
      ...
      ${page==='channel'?html`<${ChannelPage} data=${channelData} videos=${channelVideos} loading=${channelLoading} session=${session} onBack=${function(){navTo(channelFromPage||'search');}} onWatch=${handleWatch} onChannel=${handleChannel} onSubFeed=${handleSubFeed} initialTab=${channelInitialTab}/>`:null}
    </main>
