...
// ── VideoGrid with Load More button + IntersectionObserver fallback ─────────
const GRID_PAGE = 24;
...
function VideoGrid(props) {
  ...
  // Only reset page when list shrinks (new search/period) not when it grows (loading more)
  useEffect(function() {
    var newLen = videos.length;
    var oldLen = prevLenRef.current;
  ...
  return html`<div>
    ...
    ${hasMore ? html`
      ...
      <div ref=${sentinelRef} style=${{height:'1px',margin:'-1px'}}/>
      <!-- Always-visible Load More button -->
      <div style=${{display:'flex',flexDirection:'column',alignItems:'center',gap:10,padding:'32px 0 16px'}}>
        <div style=${{color:'#555',fontSize:13}}>${shown.length} of ${videos.length} videos</div>
        <button onClick=${loadMore}
          style=${{background:'#1a1a1a',border:'1px solid #00FF07',color:'#00FF07',
            padding:'12px 40px',fontSize:14,fontWeight:600,cursor:'pointer',borderRadius:0,
    ...
    ${props.loading ? html`
      <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'24px 16px',marginTop:16}}>
        ${[0,1,2,3].map(function(i){return html`<${SkeletonCard} key=${i}/>`;})}
      </div>
    ` : null}
  </div>`;
}
...
// ── Trending Page ─────────────────────────────────────────────────────────────
// Fetches as many video posts as possible across many search terms + cursors,
// then sorts all of them by like count descending.
const TRENDING_TERMS = ['video','vlog','watch','clip','stream','shorts','reel','furry','gaming','music','art','funny','tutorial','review','podcast'];
const WHATS_HOT = 'at://did:plc:z72i7hdynmk6r22z27h6tvur/app.bsky.feed.generator/whats-hot';
const DISCOVER  = 'at://did:plc:tenurhgjptubkk5zf5xxn4wv/app.bsky.feed.generator/discover';

function TrendingPage(props) {
  const [period,   setPeriod]   = useState('24h');
  const [videos,   setVideos]   = useState([]);
  const [loading,  setLoading]  = useState(true);
  const [fetching, setFetching] = useState(false); // background extra-fetch
  ...
  useEffect(function(){
    ...
    async function load() {
      ...
      const now    = Date.now();
      const ms     = {'24h':86400000,'week':604800000,'month':2592000000,'year':31536000000};
      ...
      function add(posts) {
        (posts||[]).forEach(function(p) {
          ...
          // Only include posts with a playlist URL (fully hydrated) AND within time window
          if (!p.embed || !p.embed.playlist) return;
          if (new Date(p.indexedAt).getTime() >= cutoff) {
            found.push(p); seen.add(p.uri);
          }
        });
      }

      function sortAndSet() {
        const sorted = found.slice().sort(function(a,b){ return (b.likeCount||0)-(a.likeCount||0); });
        if (!cancelled) setVideos(sorted);
      }

      // Helper: fetch N pages from a getFeed endpoint using cursor pagination
      async function fetchFeedPages(feedUri, pages) {
        let cursor = null;
        for (let pg = 0; pg < pages && !cancelled; pg++) {
          const url = PUB_PROXY+'/app.bsky.feed.getFeed?feed='+encodeURIComponent(feedUri)+'&limit=100'+(cursor?'&cursor='+encodeURIComponent(cursor):'');
          try {
            const r = await api(url);
            if (!r.ok) break;
            const d = await r.json();
            add((d.feed||[]).map(function(i){return i.post;}));
            cursor = d.cursor;
            if (!cursor) break;
          } catch(e) { break; }
          sortAndSet();
      ...
      try {
        ...
        // Quick first load — show something immediately
        await fetchFeedPages(WHATS_HOT, 2);
        sortAndSet();
        if (!cancelled) setLoading(false);

        // Background: keep fetching until we have enough videos in the window
        // or hit a hard cap to avoid infinite requests
        const targetVideos = 100;
        const maxRounds    = 30; // max 30 extra pages per feed
        let   rounds       = 0;

        while (!cancelled && found.length < targetVideos && rounds < maxRounds) {
          await fetchFeedPages(WHATS_HOT,   1);
          await fetchFeedPages(DISCOVER,    1);
          sortAndSet();
          rounds++;
          // If we're getting posts older than our window for 3 rounds in a row, stop
          if (rounds > 5 && found.length === 0) break;
        }
  ...
  return html`<div style=${{padding:24}}>
    <div style=${{display:'flex',alignItems:'center',gap:12,marginBottom:24,flexWrap:'wrap'}}>
      ...
      <div style=${{marginLeft:'auto',display:'flex',alignItems:'center',gap:10}}>
        ${fetching?html`<span style=${{color:'#00FF07',fontSize:12,animation:'pulse 1s infinite'}}>⟳ Loading more…</span>`:null}
        ${!loading?html`<span style=${{color:'#555',fontSize:13}}>${videos.length} videos</span>`:null}
      </div>
    </div>
...
function ChannelPage(props) {
  ...
  const [profileData, setProfileData] = useState(null); // override after edit
