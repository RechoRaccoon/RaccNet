...
const CHIPS = ['All','Gaming','Music','Tech','Art','Vlogs','News','Funny'];
function HomePage(props) {
  const [active, setActive] = useState('All');
  return html`<div style=${{padding:24}}>
    <div style=${{display:'flex',gap:8,overflowX:'auto',marginBottom:24,paddingBottom:4}}>
      ${CHIPS.map(function(chip){return html`<button key=${chip}
        onClick=${function(){setActive(chip);if(chip!=='All')props.onExplore(chip);}}
        style=${{flexShrink:0,padding:'6px 12px',borderRadius:8,border:'none',
          background:active===chip?'#f1f1f1':'#272727',color:active===chip?'#0f0f0f':'#f1f1f1',
          fontSize:14,fontWeight:active===chip?500:400}}>${chip}</button>`;
      })}
    </div>
...
function App() {
  ...
  const loadHome=useCallback(async function(sess){
    setHomeLoading(true);
  ...
  useEffect(function(){if(page==='home')loadHome(session);},[page]);
  ...
  return html`<div style=${{minHeight:'100vh',background:'#0f0f0f',color:'#f1f1f1'}}>
    ...
    <main style=${{marginLeft:mL,marginTop:56,minHeight:'calc(100vh - 56px)',transition:'margin-left 0.15s ease'}}>
      ${page==='home'?html`<${HomePage} videos=${homeVideos} loading=${homeLoading} onWatch=${handleWatch} onChannel=${handleChannel} onExplore=${handleSearch}/>`:null}
