...
:root{--accent:#00FF07;--accent-dim:rgba(0,255,7,0.12);--accent-dim-dark:rgba(0,255,7,0.08);--accent-solid-dim:#003300;--page-bg:#0f0f0f}
...
(function(){
  try{
    ...
    if(bg){
      ...
      s.id='raccnet-bg-style';
      s.textContent='body,#app{background:url('+bg+') center/cover fixed no-repeat !important;}';
...
function applyBg(url){
  var s=document.getElementById('raccnet-bg-style');
  if(!s){s=document.createElement('style');s.id='raccnet-bg-style';document.head.appendChild(s);}
  if(url){
    s.textContent='body,#app{background:url('+url+') center/cover fixed no-repeat !important;}';
  } else {
    s.textContent='';
  }
}
