...
function VideoCard(props) {
  ...
  function onEnter() {
    // Short delay before starting preview so quick mouse-overs don't trigger it
  ...
  return html`<div style=${{cursor:'pointer'}} onClick=${function(){props.onWatch(post);}}>
    <div style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative'}}
      ...
      ${hovering?html`<video ref=${videoRef} muted playsinline loop
        style=${{position:'absolute',top:0,left:0,width:'100%',height:'100%',objectFit:'cover',
          opacity:previewReady?1:0,transition:'opacity 0.3s'}}/>`:null}
      <!-- "Preview" badge -->
...
// Attaches an IntersectionObserver to a sentinel div at the bottom of the list.
// Calls onLoadMore() whenever the sentinel scrolls into view.
...
// ── VideoGrid with infinite scroll ───────────────────────────────────────────
function VideoGrid(props) {
  const PAGE = 24; // cards per page
  const [page, setPage] = useState(1);

  // Reset paging when video list changes completely
  useEffect(function() { setPage(1); }, [props.videos]);

  const visible = (props.videos || []).slice(0, page * PAGE);
  const hasMore  = visible.length < (props.videos || []).length;

  const loadMore = useCallback(function() {
    if (hasMore) setPage(function(p) { return p + 1; });
  }, [hasMore]);

  const sentinelRef = useInfiniteScroll(loadMore, hasMore && !props.loading);

  if (props.loading && (!props.videos || !props.videos.length)) {
  ...
  return html`<div>
    <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'24px 16px'}}>
      ${visible.map(function(p,i){return html`<${VideoCard} key=${p.uri||i} post=${p} onWatch=${props.onWatch} onChannel=${props.onChannel}/>`;})}
    </div>
    <!-- Sentinel + loading indicator -->
    <div ref=${sentinelRef} style=${{height:1}}/>
    ${hasMore?html`<div style=${{textAlign:'center',padding:'24px 0',color:'#555',fontSize:13}}>Loading more…</div>`:null}
    ${props.loadingMore?html`<div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'24px 16px',marginTop:16}}>
  ...
  useEffect(function(){
    ...
    async function load() {
      ...
      try {
        ...
        // Fetch all curated feeds in parallel (first pages fast)
        await Promise.all([
          fetchFeedPages(WHATS_HOT, 1),
          fetchFeedPages(DISCOVER,  1),
          fetchFeedPages('at://did:plc:z72i7hdynmk6r22z27h6tvur/app.bsky.feed.generator/with-friends', 1),
        ]);
        sortAndSet();
        if (!cancelled) setLoading(false);

        // Then paginate deep into each feed sequentially
        await fetchFeedPages(WHATS_HOT, 20);
        await fetchFeedPages(DISCOVER,  20);
        await fetchFeedPages('at://did:plc:z72i7hdynmk6r22z27h6tvur/app.bsky.feed.generator/with-friends', 10);
