...
function FriendCard(props) {
  ...
  return html`<div style=${{cursor:'pointer'}}
    onMouseEnter=${function(){setCardHov(true);}}
    onMouseLeave=${function(){setCardHov(false);}}>
    <div style=${{display:'flex',alignItems:'center',gap:8,marginBottom:6,height:34,overflow:'hidden'}}>
      ...
      ${msgText?html`<span style=${{color:'#888',fontSize:12,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1,minWidth:0}}>${open?'':short}</span>`:html`<span style=${{color:'#555',fontSize:12,fontStyle:'italic',flex:1}}>shared a video</span>`}
      ${needsMore?html`<span onClick=${function(e){e.stopPropagation();setOpen(function(v){return !v;});}} style=${{color:'#00FF07',fontSize:11,fontWeight:600,cursor:'pointer',flexShrink:0,marginLeft:4}}>${open?'Close':'More'}</span>`:null}
    ...
    <div onClick=${function(){if(!open)props.onWatch&&props.onWatch(post);}}
      ...
      ${open?html`<div style=${{position:'absolute',top:0,left:0,right:0,bottom:0,
        background:'rgba(0,0,0,0.82)',display:'flex',alignItems:'center',justifyContent:'center',padding:16,zIndex:2}}
        onClick=${function(e){e.stopPropagation();}}>
        <p style=${{color:'#f1f1f1',fontSize:14,lineHeight:1.6,margin:0,textAlign:'left',wordBreak:'break-word'}}>${msgText}</p>
      </div>`:null}
    </div>
