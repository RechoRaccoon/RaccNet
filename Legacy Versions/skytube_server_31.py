...
function VideoCard(props) {
  ...
  useEffect(function() {
    if (!hovering || !videoRef.current || !embed.playlist) return;
...
function isVidRaw(p) {
  if (!p) return false;
  if (isVid(p)) return true; // already hydrated
  const rec = p.record || p.value;
  if (!rec) return false;
  const embed = rec.embed;
  if (!embed) return false;
  const t = embed['$type'] || '';
  return t === 'app.bsky.embed.video' || t.indexOf('video') !== -1;
}
