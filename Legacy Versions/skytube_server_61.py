...
function ChannelDMsTab(props) {
  ...
  useEffect(function(){
    ...
    async function load(){
      ...
      try{
        ...
        var withContent=allMsgs.filter(function(m){return m.text||m.embed;});
        withContent.reverse();
  ...
  if(!msgs.length) return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>No shared videos or posts in your DMs with this channel.</div>`;
  ...
  return html`<div style=${{maxWidth:680,margin:'0 auto',display:'flex',flexDirection:'column',gap:0}}>
    ${msgs.map(function(m,i){
      ...
      return html`<div key=${i} style=${{display:'flex',flexDirection:'column',
        alignItems:isMine?'flex-end':'flex-start',padding:'8px 16px'}}>
        <div style=${{fontSize:11,color:'#444',marginBottom:4}}>${isMine?'You':'Channel'} · ${ago(m.sentAt)}</div>
        ${m.text?html`<div style=${{background:isMine?'#003300':'#1a1a1a',color:'#f1f1f1',
          padding:'10px 14px',fontSize:14,lineHeight:1.5,maxWidth:'80%',
          borderRadius:0,border:'1px solid '+(isMine?'#00FF07':'#2a2a2a')}}>
          ${m.text}
        </div>`:null}
        ${post?html`<div style=${{marginTop:m.text?8:0,width:'100%',maxWidth:440}}>
          ${isVideo
            ...
            : html`<div style=${{background:'#141414',border:'1px solid #222',padding:16,cursor:'pointer'}}
                ...
                <div style=${{fontSize:14,color:'#ddd',lineHeight:1.5,whiteSpace:'pre-wrap'}}>${(post.record&&post.record.text)||''}</div>
              </div>`
          }
        </div>`:null}
      </div>`;
