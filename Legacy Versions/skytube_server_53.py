...
function ShareModal(props) {
  ...
  async function getChatToken() {
    // Request a service auth token from our PDS, scoped to api.bsky.chat
    // aud must be the chat service DID, not our own PDS DID
    var saR = await api(AUTH_PROXY+'/com.atproto.server.getServiceAuth?aud=did%3Aweb%3Aapi.bsky.chat&lxm=chat.bsky.convo.sendMessage',
      {headers:{Authorization:'Bearer '+sess.accessJwt}});
    if(!saR.ok) {
      saR = await api(AUTH_PROXY+'/com.atproto.server.getServiceAuth?aud=did%3Aweb%3Aapi.bsky.chat',
        {headers:{Authorization:'Bearer '+sess.accessJwt}});
    }
    if(!saR.ok) {
      var errText = await saR.text();
      throw new Error('Chat auth failed: '+errText);
    }
...
function ChannelPage(props) {
  ...
  async function loadLikes(did) {
    ...
    try {
      // getActorLikes is public — use public API
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            self._proxy("GET", "https://api.bsky.chat/" + self.path[len("/proxy/chat/"):])
    ...
    def do_POST(self):
        if self.path.startswith("/proxy/auth/"):
            ...
            length = int(self.headers.get("Content-Length", 0))
            body   = self.rfile.read(length) if length else b""
            self._proxy("POST", "https://api.bsky.chat/" + self.path[len("/proxy/chat/"):], body=body)
        elif self.path.startswith("/proxy/video/"):
            length = int(self.headers.get("Content-Length", 0))
            body   = self.rfile.read(length) if length else b""
