...
function VideoPlayer({ playlist, thumbnail }) {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      if (window.Hls && window.Hls.isSupported()) {
        const hls = new window.Hls({
          enableWorker: false,       // required for Firefox CSP in sandboxed iframes
        ...
        hls.on(window.Hls.Events.ERROR, (_, data) => {
          if (data.fatal) {
            // Try direct src fallback on fatal HLS error
        ...
        // Last resort: set src directly and hope for the best
        ref.current.src = playlist;
      }
    };
    ...
    if (window.Hls) {
      ...
      script.onload = init;
      script.onerror = () => {
        // If CDN blocked (e.g. by Firefox extension), try direct src fallback
        if (ref.current) ref.current.src = playlist;
      };
...
function SkeletonCard() {
  return (
    <div>
      <div className="skt-shimmer" style={{ width: "100%", aspectRatio: "16/9", borderRadius: 12, background: "#272727" }} />
      <div style={{ display: "flex", gap: 12, padding: "12px 0" }}>
        <div className="skt-shimmer" style={{ width: 36, height: 36, borderRadius: "50%", background: "#272727", flexShrink: 0 }} />
        <div style={{ flex: 1 }}>
          <div className="skt-shimmer" style={{ height: 14, background: "#272727", borderRadius: 4, marginBottom: 8, width: "90%" }} />
          <div className="skt-shimmer" style={{ height: 12, background: "#272727", borderRadius: 4, width: "60%" }} />
        </div>
      </div>
    </div>
  );
    ...
    return (
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="line-clamp-2" style={{ fontSize: 13, fontWeight: 500, color: "#f1f1f1" }}>{title}</div>
          <div
  ...
  return (
    <div style={{ cursor: "pointer" }} onClick={() => onWatch(post)}>
      ...
      <div style={{ display: "flex", gap: 12, padding: "12px 0 4px" }}>
        ...
        />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="line-clamp-2" style={{ fontSize: 14, fontWeight: 500, color: "#f1f1f1" }}>{title}</div>
          <div
            onClick={(e) => { e.stopPropagation(); onChannel(author.handle); }}
    ...
    <div style={{ fontFamily: "'Roboto', sans-serif", background: "#0f0f0f", minHeight: "100vh", color: "#f1f1f1" }}>
      <style>{`
        ...
        * { scrollbar-width: thin; scrollbar-color: #3f3f3f transparent; }
        ...
        .skt-shimmer { animation: shimmer 1.5s ease-in-out infinite; }
        /* Firefox line-clamp support */
        .line-clamp-2 {
          display: -webkit-box;
          -webkit-line-clamp: 2;
        ...
        /* Firefox: remove default button/input appearance */
