...
function ChannelDMsTab(props) {
    ...
    async function load(){
      ...
      try{
        ...
        var cr=await api(chatUrl('/chat.bsky.convo.getConvoForMembers')+'&members[]='+encodeURIComponent(sess.did)+'&members[]='+encodeURIComponent(channelDid),
          {headers:{Authorization:'Bearer '+sess.accessJwt}});
        if(!cr.ok){
          if(cr.status===404){if(!cancelled){setLoading(false);setErr('no_convo');} return;}
  ...
  if(err==='no_convo'||!msgs.length) return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>No messages with this channel yet.</div>`;
