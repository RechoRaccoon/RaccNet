  ...
  return html`<header style=${{position:'fixed',top:0,left:0,right:0,height:56,background:'#0f0f0f',
    ...
    <div style=${{flex:1,display:'flex',justifyContent:'center'}}>
      <form onSubmit=${submit} style=${{display:'flex',width:'100%',maxWidth:600}}>
        ...
        <button type="submit"
          style=${{width:64,height:40,background:'#272727',border:'1px solid #00FF07',borderLeft:'none',
...
function FriendCard(props) {
  ...
  const MAX=50;
  const truncated=msgText.length>MAX?msgText.slice(0,MAX)+'…':msgText;
  const author=post.author||{};
  const rec=post.record||{};
  ...
  const [hov,setHov]=useState(false);
  return html`<div style=${{background:'#141414',border:'1px solid #1e1e1e',display:'flex',flexDirection:'column',cursor:'pointer',
      outline:hov?'2px solid #00FF07':'2px solid transparent',transition:'outline 0.15s'}}
    onMouseEnter=${function(){setHov(true);}} onMouseLeave=${function(){setHov(false);}}>
    <div style=${{padding:'10px 12px 6px',display:'flex',alignItems:'center',gap:8,height:44,boxSizing:'border-box',overflow:'hidden'}}>
      ...
      <div style=${{minWidth:0,flex:1,overflow:'hidden'}}>
        ...
        ${truncated?html`<span style=${{color:'#666',fontSize:11,marginLeft:6}}>${truncated}</span>`:html`<span style=${{color:'#555',fontSize:11,marginLeft:6,fontStyle:'italic'}}>shared a video</span>`}
      </div>
      <div style=${{color:'#444',fontSize:10,flexShrink:0}}>${ago(item.sentAt)}</div>
    </div>
    <div onClick=${function(){props.onWatch&&props.onWatch(post);}}>
      <div style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative'}}>
        <div style=${{position:'absolute',top:0,left:0,right:0,bottom:0}}><${Thumb} src=${thumb}/></div>
      </div>
    </div>
    <div style=${{display:'flex',gap:10,padding:'8px 10px',alignItems:'flex-start'}} onClick=${function(){props.onWatch&&props.onWatch(post);}}>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #00FF07'}}>
      <div style=${{display:'flex',alignItems:'center',gap:20,padding:'20px 0 20px'}}>
        ...
        <div style=${{flex:1,minWidth:0}}>
          ...
          <div style=${{color:'#aaa',fontSize:14}}>@${d.handle} · ${fmt(d.followersCount||0)} followers · ${fmt(d.followsCount||0)} following${isOwn&&mutualCount!==null?' · '+fmt(mutualCount)+' friends':''} · ${props.videos.length} videos</div>
