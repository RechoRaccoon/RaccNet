...
function TrendingPage(props) {
  ...
  useEffect(function(){
    ...
    async function load() {
      ...
      // How many feed pages to fetch per period (more pages = older content)
      const pageMap = {'24h': 3, 'week': 15, 'month': 50, 'year': 100};
      const pages   = pageMap[period] || 5;

      try {
        if (!cancelled) setFetching(true);

        // Show first results quickly
        await Promise.all([
        ...
        // Then go deeper — more pages for longer time windows
        await fetchFeedPages(WHATS_HOT, pages);
        sortAndSet();
        await fetchFeedPages(DISCOVER, pages);
        sortAndSet();
        await fetchFeedPages('at://did:plc:z72i7hdynmk6r22z27h6tvur/app.bsky.feed.generator/with-friends', Math.floor(pages/2));
        sortAndSet();
