...
function PostCard(props) {
  ...
  return html`<div
    style=${{background:'#141414',borderBottom:'2px solid #1a1a1a',cursor:'pointer',
      transition:'background 0.1s'}}
    onClick=${function(){props.onOpenPost&&props.onOpenPost(item);}}
    onMouseEnter=${function(e){e.currentTarget.style.background='#181818';}}
    onMouseLeave=${function(e){e.currentTarget.style.background='#141414';}}>

    ${isRepost?html`
      <div style=${{padding:'10px 24px 0',display:'flex',alignItems:'center',gap:8,color:'#666',fontSize:12}}>
        ...
        <span>${reposter.displayName||reposter.handle||'Someone'} reposted</span>
      </div>
    `:null}
    ...
    <div style=${{padding:'18px 24px'}}>
      ...
      <div style=${{display:'flex',alignItems:'center',gap:12,marginBottom:14}}>
        ...
        <div onClick=${function(e){e.stopPropagation();}}>
          <${SubscribeButton} did=${author.did} viewer=${author.viewer} session=${sess} small=${true}/>
        </div>
      </div>
      ...
      ${imgs&&imgs.length?html`<div style=${{display:'grid',
        ...
        ${imgs.slice(0,4).map(function(img,ii){return html`<img key=${ii}
          ...
          onClick=${function(e){e.stopPropagation();window.open(img.fullsize,'_blank');}}/>`;})}
      </div>`:null}

      <!-- Actions -->
      <div style=${{display:'flex',alignItems:'center',gap:24,marginTop:4}} onClick=${function(e){e.stopPropagation();}}>
        <button onClick=${toggleLike}
          style=${{background:'none',border:'none',color:liked?'#00FF07':'#555',cursor:sess?'pointer':'default',
            display:'flex',alignItems:'center',gap:6,fontSize:13,padding:0}}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill=${liked?'#00FF07':'currentColor'}>
            <path d="M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2z"/>
          </svg>
          ${fmt(likeCount)}
        </button>
        <span style=${{color:'#333',fontSize:13,display:'flex',alignItems:'center',gap:6}}>
...
function PostDetailPage(props) {
  ...
  return html`<div style=${{display:'flex',gap:24,padding:24,maxWidth:1200,margin:'0 auto'}}>
    <div style=${{flex:1,minWidth:0}}>
      <!-- Back -->
      <button onClick=${props.onBack}
        style=${{background:'none',border:'1px solid #3f3f3f',color:'#f1f1f1',padding:'7px 16px',
          marginBottom:20,cursor:'pointer',fontSize:13,display:'flex',alignItems:'center',gap:6,borderRadius:0}}
        onMouseEnter=${function(e){e.currentTarget.style.background='#272727';}}
        onMouseLeave=${function(e){e.currentTarget.style.background='none';}}>
      ...
      <div style=${{background:'#141414',border:'1px solid #222',padding:24,marginBottom:24}}>
        <div style=${{display:'flex',alignItems:'center',gap:12,marginBottom:16,cursor:'pointer'}}
          ...
          <${Avatar} src=${author.avatar} size=${52}/>
          <div>
            <div style=${{color:'#f1f1f1',fontWeight:700,fontSize:17}}>${author.displayName||author.handle}</div>
            <div style=${{color:'#555',fontSize:13}}>@${author.handle} · ${ago(post.indexedAt)}</div>
          </div>
        </div>
        ${rec.text?html`<div style=${{fontSize:17,color:'#e8e8e8',lineHeight:1.75,whiteSpace:'pre-wrap',
          marginBottom:16,wordBreak:'break-word'}}>${rec.text}</div>`:null}
        <div style=${{display:'flex',gap:24,color:'#555',fontSize:13,paddingTop:12,borderTop:'1px solid #1e1e1e'}}>
          <span>♥ ${fmt(post.likeCount||0)} likes</span>
          <span>↩ ${fmt(post.repostCount||0)} reposts</span>
          <span>💬 ${fmt(post.replyCount||0)} replies</span>
        </div>
      </div>

      <!-- Replies -->
      <h3 style=${{color:'#f1f1f1',fontSize:16,fontWeight:600,marginBottom:16}}>${fmt(post.replyCount||0)} Replies</h3>
      ${replies.length===0?html`<div style=${{color:'#555',fontSize:14}}>No replies yet.</div>`:
        replies.map(function(r,i){
          ...
          return html`<div key=${i} style=${{display:'flex',gap:12,padding:'14px 0',borderBottom:'1px solid #1a1a1a'}}>
            <${Avatar} src=${rp.author.avatar} size=${36}
              onClick=${function(){props.onChannel(rp.author.handle);}}/>
            <div style=${{flex:1}}>
              <div style=${{fontSize:13,fontWeight:600,color:'#f1f1f1',marginBottom:4}}>
                ${rp.author.displayName||rp.author.handle}
                <span style=${{color:'#555',fontWeight:400,marginLeft:8}}>${ago(rp.indexedAt)}</span>
              </div>
              <div style=${{fontSize:14,color:'#ddd',lineHeight:1.6,whiteSpace:'pre-wrap'}}>${rc.text}</div>
            </div>
          </div>`;
        })
      }
...
function ChannelPostsFeed(props) {
  ...
  const [filterOwn,   setFilterOwn]   = useState(false); // false=all, true=own only
...
function ChannelPage(props) {
  ...
  async function openTab(t) {
    ...
    if (t === 'Posts' && !postsLoaded && d && d.did) {
      ...
      try {
        // Use no filter so we get posts AND reposts (feed items have reason for reposts)
        var r = await api(PUB_PROXY+'/app.bsky.feed.getAuthorFeed?actor='+encodeURIComponent(d.did)+'&limit=100');
        if (r.ok) {
          var data = await r.json();
          // Store full feed items (not just post) so we keep repost reason
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #272727'}}>
      ...
      <div style=${{display:'flex'}}>
        ${['Videos','Posts'].map(function(t){return html`<button key=${t} onClick=${function(){openTab(t);}}
