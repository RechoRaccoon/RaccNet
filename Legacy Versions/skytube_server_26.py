...
function TrendingPage(props) {
  ...
  useEffect(function(){
    ...
    async function load() {
      ...
      try {
        // Round 1: fast parallel fetch across all terms + curated feeds
        const round1 = await Promise.all([
          // Search terms — sort=top gives highest-liked posts first
          ...TRENDING_TERMS.map(function(t){
          ...
          // Curated feeds
          api(PUB_PROXY+'/app.bsky.feed.getFeed?feed='+encodeURIComponent(WHATS_HOT)+'&limit=100')
            .then(function(r){return r.ok?r.json():{feed:[]};}).then(function(d){return (d.feed||[]).map(function(i){return i.post;});}).catch(function(){return[];}),
          api(PUB_PROXY+'/app.bsky.feed.getFeed?feed='+encodeURIComponent(DISCOVER)+'&limit=100')
            .then(function(r){return r.ok?r.json():{feed:[]};}).then(function(d){return (d.feed||[]).map(function(i){return i.post;});}).catch(function(){return[];})
        ]);
        round1.forEach(add);
        sortAndSet();
        ...
        // Round 2: deeper fetch using cursor pagination for first 6 terms
        if (!cancelled) setFetching(true);
        const deepTerms = TRENDING_TERMS.slice(0, 6);
        for (let i = 0; i < deepTerms.length && !cancelled; i++) {
          const t = deepTerms[i];
          // Fetch 3 extra pages per term
          let cursor = null;
          for (let pg = 0; pg < 3 && !cancelled; pg++) {
            const url = PUB_PROXY+'/app.bsky.feed.searchPosts?q='+encodeURIComponent(t)+'&limit=100&sort=top'+(cursor?'&cursor='+encodeURIComponent(cursor):'');
            try {
      ...
      } catch(e) { console.error('trending:',e); if(!cancelled){setLoading(false);setFetching(false);} }
    }
    load();
