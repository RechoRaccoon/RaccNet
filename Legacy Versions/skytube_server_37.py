...
function FeedPage(props) {
  ...
  async function openTab(t) {
    setTab(t);
    if (t === 'Posts' && !postsLoaded && props.feedUri) {
      ...
      try {
        ...
        const r = await api(endpoint+'/app.bsky.feed.getFeed?feed='+encodeURIComponent(props.feedUri)+'&limit=100', authOpts);
        if (r.ok) {
          const d = await r.json();
          // All items (not just video) — store full feed items for ChannelPostsFeed
          setFeedPosts(d.feed || []);
        }
...
/>`;})}
      ...
      <!-- Actions: like on left, subscribe on right -->
