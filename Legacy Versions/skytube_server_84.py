...
function SettingsPage(props) {
  const sess = props.session;
  return html`<div style=${{padding:24,maxWidth:600}}>
    <h2 style=${{color:'#f1f1f1',fontSize:22,fontWeight:700,marginBottom:24}}>Settings</h2>
    ${sess?html`<div style=${{background:'#141414',border:'1px solid #1e1e1e',padding:20,marginBottom:16}}>
      ...
      <button onClick=${props.onLogout}
        style=${{background:'none',border:'1px solid #ff4444',color:'#ff4444',
          padding:'10px 24px',fontSize:14,fontWeight:600,cursor:'pointer',borderRadius:0,width:'100%',textAlign:'left'}}
        onMouseEnter=${function(e){e.currentTarget.style.background='rgba(255,68,68,0.1)';}}
        onMouseLeave=${function(e){e.currentTarget.style.background='none';}}>
        Sign out of @${sess.handle}
      </button>
    </div>`:null}
    <div style=${{background:'#141414',border:'1px solid #1e1e1e',padding:20,marginBottom:16}}>
      <div style=${{color:'#f1f1f1',fontSize:15,fontWeight:600,marginBottom:8}}>About RaccNet</div>
      <div style=${{color:'#555',fontSize:13,lineHeight:1.7}}>
        RaccNet is a Bluesky video platform built on the AT Protocol.<br/>
    ...
    <div ref=${scrollRef} style=${{flex:1,overflowY:'auto',padding:'8px 0'}}>
      ...
      ${msgs.map(function(m,i){
        ...
        return html`<div key=${i} style=${{display:'flex',flexDirection:'column',
          ...
          ${hasText?html`<div style=${{background:isMine?'rgba(0,255,7,0.12)':'#1e1e1e',color:'#f1f1f1',
            padding:'9px 13px',fontSize:14,lineHeight:1.5,maxWidth:'72%',wordBreak:'break-word',
            border:'1px solid '+(isMine?'#00FF07':'#2a2a2a'),borderRadius:2}}>
            ${m.text}
          </div>`:null}
          ...
          ${isRecordEmbed&&post?html`<div style=${{marginTop:hasText||imgs.length?6:0,maxWidth:400,width:'100%'}}>
            ${isVideo
              ? html`<div style=${{cursor:'pointer'}} onClick=${function(){props.onWatch&&props.onWatch(post);}}>
                  <div style=${{width:'100%',paddingBottom:'56.25%',position:'relative',overflow:'hidden',background:'#1a1a1a',transition:'outline 0.15s'}}
                    onMouseEnter=${function(e){e.currentTarget.style.outline='2px solid #00FF07';}}
              ...
              : html`<div style=${{background:'#141414',border:'1px solid #2a2a2a',padding:14,cursor:'pointer',borderRadius:2}}
                  ...
                  ${post.record&&post.record.text?html`<div style=${{fontSize:13,color:'#ccc',lineHeight:1.5,whiteSpace:'pre-wrap'}}>${post.record.text.slice(0,200)}</div>`:null}
                </div>`}
          </div>`:null}
          ${isRecordEmbed&&!post&&m.embed?html`<a href=${'https://bsky.app/profile/'+(m.embed.record&&m.embed.record.uri||'').split('/')[2]+'/post/'+(m.embed.record&&m.embed.record.uri||'').split('/').pop()}
            target="_blank" rel="noreferrer" style=${{color:'#00FF07',fontSize:12,marginTop:4}}>View post on Bluesky →</a>`:null}
        </div>`;
      })}
    </div>
    <div style=${{borderTop:'1px solid #00FF07',padding:'12px 16px',display:'flex',gap:10,alignItems:'flex-end',background:'#0f0f0f'}}>
      <textarea value=${text} onInput=${function(e){setText(e.target.value);}} onKeyDown=${onKey}
        placeholder="Send a message… (Enter to send, Shift+Enter for newline)"
        rows="2"
        style=${{flex:1,background:'#1a1a1a',border:'1px solid #333',color:'#f1f1f1',padding:'10px 12px',
      ...
      <button onClick=${sendMsg} disabled=${sending||!text.trim()}
        style=${{background:sending||!text.trim()?'#1a1a1a':'#00FF07',
