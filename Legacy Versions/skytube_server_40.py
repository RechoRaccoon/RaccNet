    ...
    <div style=${{display:'flex',alignItems:'center',gap:4,flexShrink:0}}>
      ...
      <div onClick=${props.onHome} style=${{display:'flex',alignItems:'center',gap:8,cursor:'pointer',userSelect:'none'}}>
          ...
          <span style=${{fontSize:20,fontWeight:800,color:'#00FF07',letterSpacing:-0.5}}>
            IDKI’m<span style=${{color:'#00FF07',fontWeight:400}}>justababyTube</span>
          </span>
        </div>
    </div>
...
function loadLastPage(){ try{return localStorage.getItem(PAGE_KEY)||'search';}catch(e){return 'search';} }
    ...
    ${feeds.length>0?html`
      ...
      ${feeds.map(function(feed){
        ...
        const feedIcon = feed.avatar
          ? html`<img src=${feed.avatar} alt="" style=${{width:24,height:24,borderRadius:'50%',objectFit:'cover',flexShrink:0}}/>`
          : FeedIco;
...
function PostCard(props) {
  ...
  return html`<div
    style=${{background:'#141414',borderBottom:'2px solid #1a1a1a',
    ...
    <div style=${{padding:'18px 24px'}}>
      <!-- Author row — clicks open channel, NOT post -->
      ...
      <!-- Text + Images — clicking opens the post -->
...
/>`;})}
      ...
      <!-- Actions: like | repost on left, subscribe on right -->
      <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:14}} onClick=${function(e){e.stopPropagation();}}>
        <div style=${{display:'flex',gap:8}}>
          <button onClick=${toggleLike}
            style=${{background:liked?'rgba(0,255,7,0.12)':'#1a1a1a',border:'1px solid '+(liked?'#00FF07':'#333'),
              color:liked?'#00FF07':'#aaa',cursor:sess?'pointer':'default',
              display:'flex',alignItems:'center',gap:8,fontSize:15,padding:'10px 18px',borderRadius:0}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill=${liked?'#00FF07':'currentColor'}>
              <path d="M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2z"/>
            </svg>
            <span style=${{fontWeight:600}}>${fmt(likeCount)}</span>
          </button>
          <button onClick=${toggleRepost}
            style=${{background:reposted?'rgba(0,255,7,0.12)':'#1a1a1a',border:'1px solid '+(reposted?'#00FF07':'#333'),
              color:reposted?'#00FF07':'#aaa',cursor:sess?'pointer':'default',
              display:'flex',alignItems:'center',gap:8,fontSize:15,padding:'10px 18px',borderRadius:0}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill=${reposted?'#00FF07':'currentColor'}>
              <path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/>
            </svg>
            <span style=${{fontWeight:600}}>${fmt(repostCount)}</span>
          </button>
        </div>
