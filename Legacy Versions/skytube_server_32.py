...
function VideoCard(props) {
  ...
  return html`<div style=${{cursor:'pointer'}} onClick=${function(){props.onWatch(post);}}>
    <div style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative'}}
      onMouseEnter=${embed.playlist?onEnter:null} onMouseLeave=${embed.playlist?onLeave:null}>
      <!-- Thumbnail (always shown, fades when preview plays) -->
      ...
      <!-- Preview video (always in DOM so ref stays valid; hidden when not hovering) -->
...
// Simple paginator — no IntersectionObserver, just a button
function Paginator(props) {
  // props: items, pageSize, renderItem, compact
  const [visible, setVisible] = props._state;
  const items   = props.items || [];
  const shown   = items.slice(0, visible);
  const hasMore = visible < items.length;

  return html`<div>
    ${shown.map(props.renderItem)}
    ${hasMore ? html`
      <div style=${{display:'flex',flexDirection:'column',alignItems:'center',gap:8,padding:'28px 0 12px'}}>
        <div style=${{color:'#555',fontSize:12}}>${shown.length} of ${items.length}</div>
        <button
          onClick=${function(){ setVisible(function(v){ return v + props.pageSize; }); }}
          style=${{background:'#111',border:'2px solid #00FF07',color:'#00FF07',
            padding:'12px 48px',fontSize:14,fontWeight:700,cursor:'pointer',borderRadius:0,
            letterSpacing:1,transition:'background 0.15s'}}
          onMouseEnter=${function(e){e.currentTarget.style.background='rgba(0,255,7,0.12)';}}
          onMouseLeave=${function(e){e.currentTarget.style.background='#111';}}>
          LOAD MORE
        </button>
      </div>
    ` : (items.length > props.pageSize ? html`
      <div style=${{textAlign:'center',padding:'20px 0',color:'#444',fontSize:12}}>
        All ${items.length} loaded
      </div>
    ` : null)}
  </div>`;
}
...
function VideoGrid(props) {
  ...
  // Reset to first page when list changes to a completely different one
  const keyRef = useRef('');
  const newKey = videos.length + '_' + (videos[0] ? videos[0].uri : '');
  if (keyRef.current !== newKey && videos.length > 0) {
    // Only reset if it's a brand new list, not growing
    const wasGrowing = keyRef.current.startsWith((videos.length - GRID_PAGE) + '_') ||
                       keyRef.current.startsWith((videos.length - 1) + '_');
    if (!wasGrowing) setVisible(GRID_PAGE);
  ...
  return html`<div>
    ...
    ${hasMore ? html`
      <div style=${{display:'flex',flexDirection:'column',alignItems:'center',gap:8,padding:'28px 0 12px'}}>
        <div style=${{color:'#555',fontSize:12}}>${shown.length} of ${videos.length} videos</div>
        <button onClick=${function(){ setVisible(function(v){ return v + GRID_PAGE; }); }}
          style=${{background:'#111',border:'2px solid #00FF07',color:'#00FF07',
            padding:'12px 48px',fontSize:14,fontWeight:700,cursor:'pointer',borderRadius:0,
            letterSpacing:1,transition:'background 0.15s'}}
          onMouseEnter=${function(e){e.currentTarget.style.background='rgba(0,255,7,0.12)';}}
    ...
    ` : (videos.length > GRID_PAGE ? html`
      <div style=${{textAlign:'center',padding:'20px 0',color:'#444',fontSize:12}}>All ${videos.length} videos loaded</div>
    ` : null)}
  </div>`;
}

// ── VideoListCompact (for Search, Watch sidebar) ──────────────────────────────
function VideoListCompact(props) {
  ...
  if (!videos.length) return props.empty || null;

  return html`<div>
    ${shown.map(function(p,i){return html`<${VideoCardCompact} key=${p.uri||i} post=${p} onWatch=${props.onWatch} onChannel=${props.onChannel}/>`;})}
    ${hasMore ? html`
      <div style=${{display:'flex',flexDirection:'column',alignItems:'center',gap:8,padding:'20px 0'}}>
        <div style=${{color:'#555',fontSize:12}}>${shown.length} of ${videos.length}</div>
        <button onClick=${function(){ setVisible(function(v){ return v + LIST_PAGE; }); }}
          style=${{background:'#111',border:'2px solid #00FF07',color:'#00FF07',
            padding:'10px 36px',fontSize:13,fontWeight:700,cursor:'pointer',borderRadius:0,
...
// ── Trending Page ─────────────────────────────────────────────────────────────
// Pulls from ALL of Bluesky via searchPosts across many terms.
// Raw embed type detection (no playlist required – WatchPage hydrates on click).
const TREND_TERMS = [
  'video','vlog','watch','clip','stream','shorts','reel',
  'furry','gaming','music','art','funny','tutorial','review',
  'podcast','animation','drawing','cosplay','dance','cooking',
  'tech','news','sports','nature','travel'
];

// Detect video post without requiring hydrated playlist URL
function isVidRaw(p) {
  if (!p) return false;
  if (isVid(p)) return true; // already hydrated with playlist
  // Check top-level embed (partially hydrated)
  if (p.embed) {
    const t = p.embed['$type'] || '';
    if (t === 'app.bsky.embed.video#view' || t === 'app.bsky.embed.video') return true;
  }
  // Check record-level embed (raw from searchPosts)
  const rec = p.record || p.value || {};
  const rembed = rec.embed || {};
  const rt = rembed['$type'] || '';
...
function TrendingPage(props) {
  ...
  const PERIODS = [
    {id:'24h',  label:'24 Hours', since: function(){ return new Date(Date.now()-86400000).toISOString(); }},
    {id:'week', label:'This Week', since: function(){ return new Date(Date.now()-604800000).toISOString(); }},
    {id:'month',label:'This Month',since: function(){ return new Date(Date.now()-2592000000).toISOString(); }},
    {id:'year', label:'This Year', since: function(){ return new Date(Date.now()-31536000000).toISOString(); }},
  ];

  useEffect(function(){
    var cancelled = false;
    async function load() {
      setLoading(true); setVideos([]); setStatus('Searching Bluesky…');
      const periodObj = PERIODS.find(function(p){return p.id===period;}) || PERIODS[0];
      const since = periodObj.since();
      const seen  = new Set();
      const found = [];
      ...
      try {
        // Fetch many search terms in parallel, filtered by date via 'since' param
        // Bluesky searchPosts supports 'since' for date filtering
        var batches = await Promise.all(TREND_TERMS.map(function(term){
          var url = PUB_PROXY+'/app.bsky.feed.searchPosts'
          ...
          return api(url)
            ...
            .then(function(d){
              // Filter by date client-side since 'since' param isn't always supported
              var cutoff = new Date(since).getTime();
              return (d.posts||[]).filter(function(p){
                return new Date(p.indexedAt||p.createdAt||0).getTime() >= cutoff;
              });
            })
        ...
        batches.forEach(add);
        sortAndSet();
        if (!cancelled) { setLoading(false); setStatus(found.length + ' videos found'); }

        // Background: get more pages per term with cursor
        if (!cancelled) setStatus('Loading more…');
        for (var ti = 0; ti < Math.min(TREND_TERMS.length, 8) && !cancelled; ti++) {
          var term = TREND_TERMS[ti];
          var cursor = null;
          for (var pg = 0; pg < 4 && !cancelled; pg++) {
            var url2 = PUB_PROXY+'/app.bsky.feed.searchPosts'
              +'?q='+encodeURIComponent(term)
              +'&limit=100&sort=top'
              +(cursor ? '&cursor='+encodeURIComponent(cursor) : '');
            try {
              var r2 = await api(url2);
              if (!r2.ok) break;
              var d2 = await r2.json();
              var cutoff2 = new Date(since).getTime();
              var pagePosts = (d2.posts||[]).filter(function(p){
                return new Date(p.indexedAt||p.createdAt||0).getTime() >= cutoff2;
              });
              // If NO posts in this page are within window, stop paginating this term
              if (pagePosts.length === 0 && pg > 0) break;
              add(pagePosts);
              cursor = d2.cursor;
              if (!cursor) break;
            } catch(e2){ break; }
          ...
          sortAndSet();
          if (!cancelled) setStatus(found.length + ' videos found');
        }
        if (!cancelled) setStatus('');

      } catch(e){ console.error('trending:',e); if(!cancelled){setLoading(false);setStatus('');} }
    }
    load();
  ...
  return html`<div style=${{padding:24}}>
    <div style=${{display:'flex',alignItems:'center',gap:12,marginBottom:24,flexWrap:'wrap'}}>
      <h2 style=${{color:'#f1f1f1',fontSize:20,fontWeight:700,margin:0}}>Trending</h2>
      <div style=${{display:'flex',gap:8}}>
        ${PERIODS.map(function(p){
          var active = period===p.id;
          return html`<button key=${p.id} onClick=${function(){setPeriod(p.id);}}
            style=${{padding:'6px 14px',borderRadius:0,border:'none',fontSize:13,
              background:active?'#00FF07':'#272727',color:active?'#0f0f0f':'#f1f1f1',
              fontWeight:active?600:400,cursor:'pointer'}}>
            ${p.label}
          </button>`;
        })}
      </div>
      ${status?html`<span style=${{color:'#555',fontSize:12,marginLeft:'auto'}}>${status}</span>`:null}
    </div>
          ...
          ${props.followStrip.map(function(actor,i){
            return html`<div key=${actor.did||i}
              ...
              <div style=${{width:80,height:80,borderRadius:'50%',overflow:'hidden',background:'#272727',
                border:actor.hasNewVideo?'3px solid #00FF07':'3px solid #3f3f3f',boxSizing:'border-box'}}>
                ${actor.avatar?html`<img src=${actor.avatar} alt="" style=${{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>`:null}
              </div>
              <span style=${{fontSize:12,color:'#ccc',textAlign:'center',width:'100%',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
...
function VideoListCompact(props) {
  ...
  useEffect(function() {
    var n = videos.length;
    if (n < prevLenRef.current || n === 0) setVisible(LIST_PAGE);
  ...
  return html`<div>
    ...
    ${hasMore?html`
      <div style=${{display:'flex',flexDirection:'column',alignItems:'center',gap:8,padding:'20px 0'}}>
        <div style=${{color:'#555',fontSize:12}}>${shown.length} of ${videos.length}</div>
        <button onClick=${function(){setVisible(function(v){return v+LIST_PAGE;})}}
          style=${{background:'#1a1a1a',border:'1px solid #00FF07',color:'#00FF07',padding:'10px 32px',
            fontSize:13,fontWeight:600,cursor:'pointer',borderRadius:0,transition:'background 0.15s'}}
  ...
  return html`<div style=${{padding:'16px 24px'}}>
    ...
    ${(filter==='All'||filter==='Videos')?html`<div>
      ...
      ${videos.length===0?html`<div style=${{color:'#aaa',fontSize:14,padding:'24px 0'}}>No videos found for "${props.query}".</div>`:
        html`<${VideoListCompact} videos=${videos} onWatch=${props.onWatch} onChannel=${props.onChannel}/>`
      }
    </div>`:null}
...
// ── ChannelPostsFeed — large readable post cards ─────────────────────────────
function ChannelPostsFeed(props) {
  ...
  if (!posts.length)  return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>No posts found.</div>`;

  return html`<div style=${{maxWidth:680,margin:'0 auto',display:'flex',flexDirection:'column',gap:0}}>
    ${shown.map(function(post,i){
      var rec    = post.record || {};
      ...
      if (embed) {
        var et = embed['$type']||'';
        if (et==='app.bsky.embed.images#view' && embed.images) {
          imgs = embed.images;
        } else if (et==='app.bsky.embed.recordWithMedia#view' && embed.media && embed.media.images) {
          imgs = embed.media.images;
        }
      }
      return html`<div key=${post.uri||i}
        style=${{background:'#141414',borderBottom:'1px solid #222',padding:'24px',
        ...
        <div style=${{display:'flex',alignItems:'center',gap:12,marginBottom:14}}>
          <${Avatar} src=${author.avatar} size=${44}
            onClick=${function(){props.onChannel&&props.onChannel(author.handle);}}/>
          <div style=${{flex:1,minWidth:0}}>
          ...
          <a href=${'https://bsky.app/profile/'+author.handle+'/post/'+post.uri.split('/').pop()}
            target="_blank" rel="noreferrer"
            style=${{color:'#333',fontSize:12,textDecoration:'none'}}
            onMouseEnter=${function(e){e.currentTarget.style.color='#00FF07';}}
            onMouseLeave=${function(e){e.currentTarget.style.color='#333';}}>↗</a>
        </div>
        <!-- Post text -->
        ${text?html`<div style=${{fontSize:16,color:'#e8e8e8',lineHeight:1.65,whiteSpace:'pre-wrap',
          marginBottom:imgs?16:0,wordBreak:'break-word'}}>
          ${text}
        </div>`:null}
        <!-- Images -->
        ${imgs&&imgs.length?html`<div style=${{display:'grid',
          gridTemplateColumns:imgs.length===1?'1fr':imgs.length===2?'1fr 1fr':'1fr 1fr',
          gap:4,marginTop:text?0:0}}>
          ${imgs.slice(0,4).map(function(img,ii){
            return html`<img key=${ii} src=${img.thumb||img.fullsize} alt=${img.alt||''}
              style=${{width:'100%',aspectRatio:imgs.length===1?'16/9':'1/1',
                objectFit:'cover',display:'block'}}
              onClick=${function(){window.open(img.fullsize,'_blank');}}/>`;
          })}
        </div>`:null}
        <!-- Stats -->
        <div style=${{display:'flex',gap:20,marginTop:14,color:'#444',fontSize:13}}>
          <span>♥ ${fmt(post.likeCount||0)}</span>
    ...
    ${hasMore?html`
      <div style=${{display:'flex',justifyContent:'center',padding:'24px 0',background:'#141414',borderBottom:'1px solid #222'}}>
        <button onClick=${function(){setVisible(function(v){return v+10;})}}
          style=${{background:'#111',border:'2px solid #00FF07',color:'#00FF07',
            padding:'12px 48px',fontSize:14,fontWeight:700,cursor:'pointer',borderRadius:0,
            letterSpacing:1}}
          onMouseEnter=${function(e){e.currentTarget.style.background='rgba(0,255,7,0.12)';}}
...
function ChannelPage(props) {
  ...
  async function openTab(t) {
    setTab(t);
    if (t === 'Posts' && !postsLoaded && d && d.did) {
      setPostsLoading(true);
      try {
        var r = await api(PUB_PROXY+'/app.bsky.feed.getAuthorFeed?actor='+encodeURIComponent(d.did)+'&limit=100&filter=posts_no_replies');
        if (r.ok) {
          var data = await r.json();
          setAllPosts((data.feed||[]).map(function(i){return i.post;}));
        }
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #272727'}}>
      ...
      <div style=${{display:'flex'}}>
        ${['Videos','Posts','About'].map(function(t){return html`<button key=${t} onClick=${function(){openTab(t);}}
    ...
    <div style=${{padding:24}}>
      ...
      ${tab==='Posts'?html`<${ChannelPostsFeed} posts=${allPosts} loading=${postsLoading} onChannel=${props.onChannel}/>`:null}
...
function App() {
  ...
  const handleChannel = useCallback(async function(actor){
    ...
    try{
      ...
      // Paginate ALL pages until cursor runs out
      const seen = new Set();
      const vids = [];
      let cursor = null;
      do {
        ...
        // Update incrementally so user sees videos as they load
