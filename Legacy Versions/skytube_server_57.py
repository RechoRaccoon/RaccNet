...
function FriendsFeed(props) {
  ...
  const [items,    setItems]    = useState([]);  // [{sender, message, post}]
  const [loading,  setLoading]  = useState(false);
    ...
    async function load(){
      ...
      try{
        ...
        for(var i=0;i<convos.length&&!cancelled;i++){
          ...
          // 3. Find messages with record embeds
          for(var j=0;j<msgs.length;j++){
            ...
            var postUri=rec.uri;
            if(!postUri) continue;
            // sender is anyone other than us
            var sender=(convos[i].members||[]).find(function(mb){return mb.did!==sess.did;}) || {};
            found.push({sender:sender, msgText:m.text||'', postUri:postUri, postCid:rec.cid, sentAt:m.sentAt||''});
          }
        }
        ...
        if(found.length>0&&!cancelled){
          ...
          if(!cancelled) setItems(videoItems);
        }
      ...
      try{
        ...
        if(r.ok){
          ...
          if(d&&d.convos&&!cancelled){
            var sorted=d.convos.slice().sort(function(a,b){
  ...
  function onSearchInput(e){
    ...
    searchTimer=setTimeout(async function(){
      ...
      try{
        ...
        if(r.ok){var d=await r.json(); setSearchRes(d.actors||[]);}
      }catch(e2){}
      setSearching(false);
    },350);
  }
...
function LikedTab(props) {
  ...
  return html`<div>
    ...
    ${sub==='Liked Posts'?html`<${ChannelPostsFeed} posts=${props.posts||[]} loading=${false} session=${props.session} onChannel=${props.onChannel} onWatch=${props.onWatch}/>`:null}
  </div>`;
}
...
function ChannelPage(props) {
  ...
  async function loadLikes(did) {
    ...
    try {
      // Try with auth first (required if account has likes hidden), fall back to public
      var url  = PUB_PROXY+'/app.bsky.feed.getActorLikes?actor='+encodeURIComponent(did)+'&limit=100';
      var opts = props.session ? {headers:{Authorization:'Bearer '+props.session.accessJwt}} : {};
      var authUrl = AUTH_PROXY+'/app.bsky.feed.getActorLikes?actor='+encodeURIComponent(did)+'&limit=100';
      var r = props.session ? await api(authUrl, opts) : await api(url);
      if (!r || !r.ok) r = await api(url); // fallback to public
      if(r.ok){
        var data  = await r.json();
        var items = data.feed||[];
        // Store all feed items for Posts tab
        setLikedPosts(items);
        // For Videos tab — any post with a video embed (raw or hydrated)
        var vids = items.map(function(i){return i.post;}).filter(function(p){
          if(!p) return false;
          if(isVid(p)) return true;
          if(isVidRaw(p)) return true;
          return false;
        });
        setLikedVids(vids);
        setLikedLoaded(true);
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #272727'}}>
      ...
      <div style=${{display:'flex'}}>
        ${(['Videos','Posts','Liked']).map(function(t){return html`<button key=${t} onClick=${function(){openTab(t);}}
