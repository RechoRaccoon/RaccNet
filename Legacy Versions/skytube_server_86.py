...
(function(){
  try{
    ...
    if(bg){document.body.style.backgroundImage='url('+bg+')';document.body.style.backgroundSize='cover';document.body.style.backgroundAttachment='fixed';}
  }catch(e){}
})();
'use strict';
...
function applyAccent(color){
  ...
  s.textContent=':root{--accent:'+color+';}';
}
    ...
    ${section('Background Image', html`<div>
      ...
      <div style=${{display:'flex',gap:10,flexWrap:'wrap'}}>
        <button onClick=${function(){bgRef.current&&bgRef.current.click();}}
          style=${{background:'#00FF07',color:'#000',border:'none',padding:'9px 18px',fontSize:13,fontWeight:700,cursor:'pointer',borderRadius:0}}>
          Choose Image
        </button>
    ...
    ${section('Content Filter', html`<div>
      ...
      ${[['all','Show all content (default)'],['sfw','Hide adult/explicit content'],['nsfw','Show only adult/explicit content']].map(function(opt){
        return html`<label key=${opt[0]} style=${{display:'flex',alignItems:'center',gap:10,padding:'8px 0',cursor:'pointer'}}>
          <input type="radio" name="filter" value=${opt[0]} checked=${filter===opt[0]}
            onChange=${function(){applyAndSaveFilter(opt[0]);}}
            style=${{accentColor:'#00FF07',width:16,height:16,cursor:'pointer'}}/>
          <span style=${{color:'#f1f1f1',fontSize:14}}>${opt[1]}</span>
        </label>`;
      })}
    </div>`)}
