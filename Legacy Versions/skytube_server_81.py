...
function ChannelDMsTab(props) {
  ...
  useEffect(function(){
    ...
    async function load(){
      ...
      try{
        // Step 1: list all convos, find one whose members include channelDid
        // (don't use getConvoForMembers — it creates a new empty convo if none exists)
        ...
        console.log('[DMs] found convo', cid, 'total msgs:', allMsgs.length);

        var withContent=allMsgs.filter(function(m){
          return (m['$type']||'')!=='chat.bsky.convo.defs#deletedMessageView';
        });
        withContent.reverse(); // oldest first for chat layout
