  ...
  return html`<div style=${{padding:24}}>
    ...
    <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:24}}>
      ${items.map(function(item,i){
        var post=item.post;
        return html`<div key=${i} style=${{background:'#141414',border:'1px solid #1e1e1e'}}>
          ${item.msgText?html`<div style=${{padding:'12px 16px 0',display:'flex',alignItems:'center',gap:10}}>
            <${Avatar} src=${item.sender.avatar} size=${28} onClick=${function(){props.onChannel&&props.onChannel(item.sender.handle);}}/>
            <div style=${{minWidth:0}}>
              <span style=${{color:'#f1f1f1',fontSize:13,fontWeight:600}}>${item.sender.displayName||item.sender.handle}</span>
              <span style=${{color:'#aaa',fontSize:12,marginLeft:8}}>${item.msgText}</span>
            </div>
          </div>`:null}
          <div style=${{padding:8}} onClick=${function(){props.onWatch&&props.onWatch(post);}}>
...
function ChannelDMsTab(props) {
  ...
  if(!msgs.length) return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>No DM messages with this channel yet.</div>`;
...
function ChannelPage(props) {
  ...
  const [dmTabKey,    setDmTabKey]    = useState(0); // force remount when channel changes
  ...
  useEffect(function() {
    if (d && d.did && d.did !== loadedDid) {
      setAllPosts([]);
      setLoadedDid(null);
      // If already on Posts tab, load immediately
