...
const SESSION_KEY = 'skytube_session';
...
function UploadModal(props) {
  ...
  const [status,     setStatus]     = useState('idle'); // idle|extracting|uploading|processing|posting|done|error
  ...
  function extractFrame(file) {
    return new Promise(function(resolve, reject) {
      ...
      vid.src = url;
      vid.addEventListener('error', function(){ URL.revokeObjectURL(url); reject(new Error('Could not read video')); });
      vid.addEventListener('loadedmetadata', function() {
        vid.currentTime = 0.001; // first possible frame
      });
  ...
  async function onVideoChange(e) {
    ...
    if (!file) return;
    setVideoFile(file);
    setThumbBlob(null); setThumbUrl(null); setError('');
    setStatus('extracting'); setProgress('Extracting first frame…');
    try {
      ...
      setError('Could not extract frame: ' + err.message);
      setStatus('idle'); setProgress('');
    }
  }
  ...
  async function upload() {
    ...
    try {
      // ── Step 1: Upload thumbnail blob ───────────────────────────────────────
      let thumbRef = null;
      if (thumbBlob) {
        setProgress('Uploading thumbnail…');
        const tRes = await fetch(AUTH_PROXY+'/com.atproto.repo.uploadBlob', {
          method:'POST',
          headers:{'Content-Type':'image/jpeg','Authorization':'Bearer '+sess.accessJwt},
          body: thumbBlob
        });
        if (!tRes.ok) throw new Error('Thumbnail upload failed: '+(await tRes.text()));
        const tData = await tRes.json();
        thumbRef = tData.blob;
      }

      // ── Step 2: Upload video to video.bsky.app ──────────────────────────────
      setProgress('Uploading video… (this may take a while)');
      const vRes = await fetch(VIDEO_PROXY+'/app.bsky.video.uploadVideo?did='+encodeURIComponent(sess.did)+'&name='+encodeURIComponent(videoFile.name), {
        method:'POST',
        headers:{'Content-Type': videoFile.type || 'video/mp4','Authorization':'Bearer '+sess.accessJwt},
        body: videoFile
      });
      ...
      let attempts = 0;
      while (attempts < 120) {
        setProgress('Processing video… '+Math.min(attempts*2,99)+'%');
      ...
      if (thumbRef) embed.thumbnail = thumbRef;
  ...
  const busy = status==='uploading'||status==='processing'||status==='posting'||status==='extracting';
  const done = status==='done';

  const iSt = {width:'100%',padding:'10px 14px',background:'#121212',border:'1px solid #3f3f3f',
    borderRadius:8,color:'#f1f1f1',fontSize:14,boxSizing:'border-box',resize:'vertical'};
  ...
  return html`<div onClick=${props.onClose}
    ...
    <div onClick=${function(e){e.stopPropagation();}}
      style=${{background:'#212121',borderRadius:16,padding:32,width:540,maxWidth:'100%',maxHeight:'90vh',
        overflowY:'auto',boxShadow:'0 16px 48px rgba(0,0,0,0.7)'}}>
      ...
      <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
        ...
        ${!busy?html`<button onClick=${props.onClose}
          style=${{background:'none',border:'none',color:'#aaa',fontSize:22,padding:'0 4px',cursor:'pointer'}}
          onMouseEnter=${function(e){e.currentTarget.style.color='#f1f1f1';}}
          onMouseLeave=${function(e){e.currentTarget.style.color='#aaa';}}>✕</button>`:null}
      </div>

      ${done ? html`
        <div style=${{textAlign:'center',padding:'32px 0',color:'#f1f1f1'}}>
          <div style=${{fontSize:56,marginBottom:12}}>🎉</div>
          <div style=${{fontSize:18,fontWeight:600,marginBottom:8}}>Video uploaded!</div>
          <div style=${{fontSize:14,color:'#aaa'}}>Your video is now live on Bluesky.</div>
        </div>
      ` : html`
        <!-- Video file picker -->
        <div style=${{marginBottom:20}}>
          <label style=${{display:'block',color:'#aaa',fontSize:13,marginBottom:8,fontWeight:500}}>Video File *</label>
          <label style=${{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',
            gap:8,padding:24,border:'2px dashed '+(videoFile?'#ff0000':'#3f3f3f'),borderRadius:12,
            cursor:'pointer',background:videoFile?'rgba(255,0,0,0.05)':'#181818',
            transition:'border-color 0.15s'}}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill=${videoFile?'#ff0000':'#555'}>
              <path d="M9 16h6v-6h4l-7-7-7 7h4v6zm-4 2h14v2H5v-2z"/>
            </svg>
            <span style=${{color:videoFile?'#f1f1f1':'#aaa',fontSize:14}}>
              ${videoFile?videoFile.name:'Click to choose a video file'}
            </span>
            ${videoFile?html`<span style=${{fontSize:12,color:'#aaa'}}>${(videoFile.size/1024/1024).toFixed(1)} MB</span>`:null}
        ...
        ${thumbUrl?html`
          <div style=${{marginBottom:20}}>
            <label style=${{display:'block',color:'#aaa',fontSize:13,marginBottom:8,fontWeight:500}}>
              Auto-generated Thumbnail (first frame)
            </label>
            <img src=${thumbUrl} style=${{width:'100%',borderRadius:10,maxHeight:200,objectFit:'cover',display:'block'}}/>
          </div>
        `:null}

        <!-- Title -->
        <div style=${{marginBottom:16}}>
          <label style=${{display:'block',color:'#aaa',fontSize:13,marginBottom:8,fontWeight:500}}>Title *</label>
          <input value=${title} onInput=${function(e){setTitle(e.target.value);}}
            placeholder="Give your video a title"
            maxlength="200"
            style=${Object.assign({},iSt,{resize:'none'})}
            disabled=${busy}
            onFocus=${function(e){e.target.style.borderColor='#ff0000';}}
            onBlur=${function(e){e.target.style.borderColor='#3f3f3f';}}/>
          <div style=${{textAlign:'right',fontSize:11,color:'#555',marginTop:4}}>${title.length}/200</div>
        </div>

        <!-- Description -->
        <div style=${{marginBottom:24}}>
          <label style=${{display:'block',color:'#aaa',fontSize:13,marginBottom:8,fontWeight:500}}>Description</label>
          <textarea value=${desc} onInput=${function(e){setDesc(e.target.value);}}
            ...
            onFocus=${function(e){e.target.style.borderColor='#ff0000';}}
            onBlur=${function(e){e.target.style.borderColor='#3f3f3f';}}/>
          <div style=${{textAlign:'right',fontSize:11,color:'#555',marginTop:4}}>${desc.length}/2000</div>
        </div>

        <!-- Post preview -->
        ${title.trim()?html`
          <div style=${{marginBottom:20,background:'#181818',borderRadius:10,padding:'12px 14px',border:'1px solid #2a2a2a'}}>
            <div style=${{fontSize:11,color:'#555',marginBottom:6,textTransform:'uppercase',letterSpacing:1}}>Post preview</div>
            <div style=${{fontSize:13,color:'#f1f1f1',lineHeight:1.6,whiteSpace:'pre-wrap'}}>
        ...
        ${busy||status==='error'?html`
          <div style=${{marginBottom:20}}>
            ${STEPS.map(function(step,i){
              const states=['uploading','processing','posting','done'];
              const curIdx=states.indexOf(status);
              const stepIdx=states.indexOf(step.id);
              const active=status===step.id;
              const complete=curIdx>stepIdx;
              const pending=curIdx<stepIdx;
              return html`<div key=${step.id} style=${{display:'flex',alignItems:'center',gap:10,padding:'6px 0',
                ...
                <div style=${{width:20,height:20,borderRadius:'50%',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',
                  background:complete?'#2a7a2a':active?'#ff0000':'#333',
                  fontSize:10,fontWeight:700,color:'#fff'}}>
                  ${complete?'✓':(i+1)}
                </div>
                <span style=${{fontSize:13,color:active?'#f1f1f1':'#aaa',fontWeight:active?600:400}}>${step.label}</span>
                ${active?html`<span style=${{marginLeft:'auto',fontSize:12,color:'#aaa'}}>${progress}</span>`:null}
              </div>`;
            })}
          </div>
        `:null}
        ...
        <!-- Error -->
        ${error?html`<div style=${{background:'#2a0000',border:'1px solid #ff4444',borderRadius:8,
          padding:'10px 14px',marginBottom:16,color:'#ff8888',fontSize:13,lineHeight:1.5}}>
          ⚠️ ${error}
        </div>`:null}

        <!-- Upload button -->
        <button onClick=${upload}
          disabled=${busy||!videoFile||!title.trim()}
          style=${{width:'100%',padding:14,background:busy?'#3f0000':'#ff0000',color:'#fff',
            border:'none',borderRadius:10,fontSize:15,fontWeight:700,
            opacity:(busy||!videoFile||!title.trim())?0.6:1,
            transition:'opacity 0.15s,background 0.15s',cursor:(busy||!videoFile||!title.trim())?'not-allowed':'pointer'}}>
          ${busy?(progress||'Working…'):'Upload Video'}
        </button>

        <p style=${{color:'#555',fontSize:12,textAlign:'center',marginTop:12}}>
          The thumbnail is automatically set to the first frame of your video.
          Your title and description are posted as text so every AT Protocol app can read them.
        </p>
      `}
    </div>
  </div>`;
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path.startswith('/proxy/auth/'):
            ...
            # Video uploads can be very large — read with a generous timeout
            length = int(self.headers.get('Content-Length', 0))
            body = self.rfile.read(length) if length else b''
    ...
    def _proxy(self, method, url, body=None, timeout=30):
