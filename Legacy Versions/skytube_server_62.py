  ...
  useEffect(function(){
    ...
    async function load(){
      ...
      try{
        ...
        var allMsgs=(await mr.json()).messages||[];
        // Keep only messages that have embeds or text, sort oldest first for chat feel
        var withContent=allMsgs.filter(function(m){return (m.text&&m.text.trim())||m.embed;});
        withContent.reverse();
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            _new_qs = '&'.join(f'{k}={v[0]}' for k,v in _qs.items() if k != '_pds')
