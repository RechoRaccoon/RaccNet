"""
IDKI’mjustababyTube local proxy server
...
<title>IDKI’mjustababyTube</title>
  ...
  return html`<aside style=${{position:'fixed',top:56,left:0,bottom:0,width:open?240:72,background:'#0f0f0f',
    ...
    ${open ? html`
      ...
      <div style=${{padding:'4px 12px'}}>
        <div style=${{color:'#aaa',fontSize:12,marginBottom:6}}>IDKI’mjustababyTube</div>
  ...
  return html`<div onClick=${props.onClose}
    ...
    <div onClick=${function(e){e.stopPropagation();}}
      ...
      <div style=${{display:'flex',alignItems:'center',gap:10,marginBottom:6}}>
        ...
        <h2 style=${{color:'#f1f1f1',fontSize:20,fontWeight:600}}>Sign in to IDKI’mjustababyTube</h2>
      </div>
...
function FriendCard(props) {
  ...
  return html`<div style=${{background:'#141414',border:'1px solid #1e1e1e',display:'flex',flexDirection:'column'}}>
    <div style=${{padding:'12px 16px 8px',display:'flex',alignItems:'flex-start',gap:10,minHeight:56}}>
      <${Avatar} src=${sender.avatar} size=${28}
        onClick=${function(){props.onChannel&&props.onChannel(sender.handle);}}/>
      <div style=${{minWidth:0,flex:1}}>
        <div style=${{color:'#f1f1f1',fontSize:13,fontWeight:600,marginBottom:2}}>${sender.displayName||sender.handle||'Unknown'}</div>
        ${msgText?html`<div style=${{color:'#aaa',fontSize:12,lineHeight:1.4,wordBreak:'break-word'}}>
          ${display}
          ${needsMore?html`<span onClick=${function(e){e.stopPropagation();setExpanded(function(v){return !v;});}}
            style=${{color:'#00FF07',cursor:'pointer',marginLeft:4,fontSize:11}}>${expanded?'less':'more'}</span>`:null}
        </div>`:html`<div style=${{color:'#555',fontSize:12,fontStyle:'italic'}}>shared a video</div>`}
      </div>
      <div style=${{color:'#444',fontSize:11,flexShrink:0,marginTop:2}}>${ago(item.sentAt)}</div>
    </div>
    <div style=${{padding:'0 8px 8px',cursor:'pointer'}} onClick=${function(){props.onWatch&&props.onWatch(post);}}>
      <div style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative'}}>
        <div style=${{position:'absolute',top:0,left:0,right:0,bottom:0}}>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #00FF07'}}>
      <div style=${{display:'flex',alignItems:'center',gap:20,padding:'20px 0 20px'}}>
        ...
        <div style=${{flex:1,minWidth:0}}>
          ...
          <div style=${{color:'#aaa',fontSize:14}}>@${d.handle} · ${fmt(d.followersCount||0)} followers · ${fmt(d.followsCount||0)} following${mutualCount!==null?' · '+fmt(mutualCount)+' friends':''} · ${props.videos.length} videos</div>
