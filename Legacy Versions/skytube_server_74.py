...
function VideoPlayer(props) {
    ...
    function setup() {
      ...
      if (window.Hls && window.Hls.isSupported()) {
        ...
        hls.on(window.Hls.Events.MANIFEST_PARSED, function() {
          if (ref.current) { ref.current.volume = loadVolume(); ref.current.play().catch(function(){}); }
        });
  ...
  function onVolumeChange(e) { saveVolume(e.target.volume); }
...
function FriendsFeed(props) {
    ...
    async function load(){
      ...
      try{
        ...
        for(var i=0;i<convos.length&&!cancelled;i++){
          var mr=await api(chatUrl('/chat.bsky.convo.getMessages')+'&convoId='+encodeURIComponent(convos[i].id)+'&limit=50',
            {headers:{Authorization:'Bearer '+sess.accessJwt}});
          if(!mr.ok) continue;
          var msgs=(await mr.json()).messages||[];
        ...
        if(found.length>0&&!cancelled){
          ...
          // 5. Build final list, keep only video posts
          var videoItems=found.map(function(x){
            return Object.assign({},x,{post:hydrated[x.postUri]||null});
          }).filter(function(x){
            if(!x.post) return false;
            return isVid(x.post)||isVidRaw(x.post);
          });
          // Sort newest first
          videoItems.sort(function(a,b){return b.sentAt.localeCompare(a.sentAt);});
          // Separate video vs non-video
          var vidOnly=videoItems.filter(function(x){return x.post&&(isVid(x.post)||isVidRaw(x.post));});
          var postOnly=videoItems.filter(function(x){return x.post&&!isVid(x.post)&&!isVidRaw(x.post);});
          // Build unique sender list
          var seenSenders=new Set();
          var senders=[];
          videoItems.forEach(function(x){
  ...
  return html`<div style=${{padding:24}}>
    ...
    ${subTab==='Posts'?html`<div>
      ${postItems.length?postItems.map(function(item,i){
        var sender=item.sender||{};
        var msgText=(item.msgText||'').trim();
        return html`<div key=${i} style=${{marginBottom:8}}>
          <div style=${{display:'flex',alignItems:'center',gap:8,padding:'6px 0 4px',borderTop:i>0?'1px solid #1a1a1a':'none'}}>
            <${Avatar} src=${sender.avatar} size=${22}
              onClick=${function(){props.onChannel&&props.onChannel(sender.handle);}}/>
            <span style=${{color:'#aaa',fontSize:12}}>
              <span style=${{color:'#e0e0e0',fontWeight:600}}>${sender.displayName||sender.handle}</span>
              ${msgText?html`<span style=${{marginLeft:6,color:'#666'}}>${msgText}</span>`:null}
              <span style=${{marginLeft:6,color:'#444'}}>${ago(item.sentAt)}</span>
            </span>
          </div>
          <${PostCard} item=${item.post} session=${props.session} onChannel=${props.onChannel} onOpenPost=${function(){}}/>
        </div>`;
      }):html`<div style=${{padding:'32px 0',color:'#555',fontSize:14}}>No posts shared with you yet.</div>`}
    </div>`:null}
  </div>`;
}
...
function SubsPage(props) {
  ...
  const [contentSub,  setContentSub]  = useState('Videos'); // sub-tab inside Content
...
function ChannelDMsTab(props) {
    ...
    async function load(){
      ...
      try{
        ...
        var withContent=allMsgs.filter(function(m){
          var t=m['$type']||'';
          return t!=='chat.bsky.convo.defs#deletedMessageView' && (m.text!==undefined||m.embed);
        });
        console.log('[DMs] withContent:', withContent.length, 'types:', withContent.map(function(m){return m['$type']+'|text='+JSON.stringify(m.text)+'|embed='+!!(m.embed);}));
        withContent.reverse();
  ...
  if(!msgs.length&&!loading) return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>
    <div>No messages with this channel yet.</div>
    <div style=${{fontSize:12,marginTop:8,color:'#555'}}>Use the Share button on a video to start a conversation.</div>
  </div>`;
