...
function Avatar(props) {
  ...
  const st = {
    ...
    outline: (clickable && hov) ? '2px solid #00FF07' : '2px solid transparent',
...
function VideoCard(props) {
  ...
  return html`<div style=${{cursor:'pointer'}} onClick=${function(){props.onWatch(post);}}>
    <div style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative',
      outline:cardHov?'2px solid #00FF07':'2px solid transparent',transition:'outline 0.15s'}}
      ...
      ${hovering&&previewReady?html`<div style=${{position:'absolute',bottom:6,right:6,
        background:'rgba(0,0,0,0.7)',color:'#00FF07',fontSize:10,padding:'2px 6px',fontWeight:600,letterSpacing:1}}>
        PREVIEW
      </div>`:null}
    </div>
    <div style=${{display:'flex',gap:12,paddingTop:12}}>
...
function Header(props) {
  ...
  return html`<header style=${{position:'fixed',top:0,left:0,right:0,height:56,background:'#0f0f0f',
    display:'flex',alignItems:'center',padding:'0 16px',gap:16,zIndex:200,borderBottom:'1px solid #00FF07'}}>
    ...
    <div style=${{flex:1,display:'flex',justifyContent:'center'}}>
      <form onSubmit=${submit} style=${{display:'flex',width:'100%',maxWidth:600}}>
        <input value=${props.input} onInput=${function(e){props.setInput(e.target.value);}}
          placeholder="Search"
          style=${{flex:1,height:40,border:'1px solid #00FF07',borderRight:'none',
          ...
          onBlur=${function(e){e.target.style.borderColor='#00FF07';}}/>
        <button type="submit"
          style=${{width:64,height:40,background:'#003300',border:'1px solid #00FF07',borderLeft:'none',
            borderRadius:0,color:'#f1f1f1',display:'flex',alignItems:'center',justifyContent:'center'}}
          onMouseEnter=${function(e){e.currentTarget.style.background='#005500';}}
          onMouseLeave=${function(e){e.currentTarget.style.background='#003300';}}>
    ...
    <div style=${{flexShrink:0,display:'flex',alignItems:'center',gap:8}}>
      ${props.session?html`
        <button onClick=${props.onUpload} title="Upload video"
          style=${{background:'#00FF07',border:'none',color:'#000',padding:'8px 16px',
            display:'flex',alignItems:'center',gap:6,fontWeight:700,fontSize:13,cursor:'pointer',borderRadius:0}}
          onMouseEnter=${function(e){e.currentTarget.style.background='#00cc06';}}
          onMouseLeave=${function(e){e.currentTarget.style.background='#00FF07';}}>
      ...
      ${props.session ? html`
        <div style=${{display:'flex',alignItems:'center',gap:12}}>
          ...
          <button onClick=${props.onLogout}
            style=${{background:'none',border:'1px solid #00FF07',color:'#f1f1f1',padding:'6px 12px',borderRadius:0,fontSize:13}}
            onMouseEnter=${function(e){e.currentTarget.style.background='#272727';}}
...
function SideItem(props) {
  return html`<button onClick=${props.onClick} title=${!props.open ? props.label : ''}
    style=${{display:'flex',alignItems:'center',gap:props.open?24:0,padding:props.open?'10px 12px':'18px 0',
      width:'100%',background:props.active?'rgba(0,255,7,0.12)':'none',border:'none',
      borderLeft:props.active?'3px solid #00FF07':'3px solid transparent',
      color:props.active?'#00FF07':'#f1f1f1',
    ...
    onMouseEnter=${function(e){if(!props.active)e.currentTarget.style.background='rgba(0,255,7,0.06)';}}
...
function Sidebar(props) {
  ...
  return html`<aside style=${{position:'fixed',top:56,left:0,bottom:0,width:open?240:72,background:'#0f0f0f',
    ...
    transition:'width 0.15s ease',boxSizing:'border-box',borderRight:'1px solid #00FF07'}}>
    ...
    ${props.hasSession ? html`
      <div style=${{height:1,background:'#00FF07',margin:'8px 0'}}/>
      ...
      <div ref=${scrollRef}
        ...
        ${(props.actors||[]).map(function(actor,i){
          return html`<div key=${actor.did||i}
            ...
            onMouseEnter=${function(e){e.currentTarget.querySelector('div').style.boxShadow='0 0 0 4px #00FF07, 0 0 12px rgba(0,255,7,0.5)';}}
            onMouseLeave=${function(e){e.currentTarget.querySelector('div').style.boxShadow='0 0 0 4px #00FF07';;}}>
            <div style=${{width:110,height:110,borderRadius:'50%',overflow:'hidden',background:'#272727',
              boxShadow:'0 0 0 4px #00FF07',
...
function FriendsFeed(props) {
    ...
    async function load(){
      ...
      try{
        ...
        for(var i=0;i<convos.length&&!cancelled;i++){
          ...
          // 3. Find messages with record embeds — only messages sent TO us (not by us)
          var convoOther=(convos[i].members||[]).find(function(mb){return mb.did!==sess.did;}) || {};
          for(var j=0;j<msgs.length;j++){
            var m=msgs[j];
            // Skip messages the logged-in user sent
            if(m.sender&&m.sender.did===sess.did) continue;
            var embed=m.embed;
            if(!embed) continue;
            var et=embed['$type']||'';
            if(et!=='app.bsky.embed.record'&&et!=='app.bsky.embed.record#view') continue;
            var rec=embed.record||{};
            var postUri=rec.uri;
            if(!postUri) continue;
            found.push({sender:convoOther, msgText:m.text||'', postUri:postUri, postCid:rec.cid, sentAt:m.sentAt||''});
          }
        }
        // 4. Hydrate all post URIs
        if(found.length>0&&!cancelled){
          var uris=found.map(function(x){return x.postUri;});
          // getPosts can take up to 25 URIs at once
          var hydrated={};
          for(var k=0;k<uris.length;k+=25){
            ...
            if(pr.ok){
              ...
              (pd.posts||[]).forEach(function(p){hydrated[p.uri]=p;});
            }
          }
          // 5. Build ALL hydrated items, then split by type
          var allHydrated=found.map(function(x){
            return Object.assign({},x,{post:hydrated[x.postUri]||null});
          }).filter(function(x){return !!x.post;});
          allHydrated.sort(function(a,b){return b.sentAt.localeCompare(a.sentAt);});
          var vidOnly=allHydrated.filter(function(x){return isVid(x.post)||isVidRaw(x.post);});
          var postOnly=allHydrated.filter(function(x){return !isVid(x.post)&&!isVidRaw(x.post);});
          // Build unique sender list from all items
          var seenSenders=new Set();
          var senders=[];
          allHydrated.forEach(function(x){
            if(x.sender&&x.sender.did&&!seenSenders.has(x.sender.did)){
              seenSenders.add(x.sender.did); senders.push(x.sender);
            }
          });
          if(!cancelled){setItems(vidOnly);setPostItems(postOnly);setSenders(senders);}
        }
  ...
  if(!items.length) return html`<div style=${{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:'40vh',gap:16,color:'#aaa'}}>
    ...
    <p style=${{fontSize:16}}>No videos shared with you yet.</p>
    <p style=${{fontSize:13,color:'#555'}}>When a friend DMs you a video, it will appear here.</p>
  </div>`;

  const tabSt2=function(a){return {padding:'6px 14px',background:a?'#00FF07':'none',color:a?'#000':'#aaa',
  ...
  return html`<div style=${{padding:24}}>
    ...
    ${subTab==='Posts'?html`<div>
      ${!postItems.length?html`<div style=${{padding:'32px 0',color:'#555',fontSize:14}}>No posts shared with you yet.</div>`:null}
      ${postItems.length?html`<${FriendPostsFeed} items=${postItems} session=${props.session} onChannel=${props.onChannel} onWatch=${props.onWatch}/>`:null}
    </div>`:null}
  </div>`;
}

// ── FriendPostsFeed — non-video posts shared via DM, with sender header ───────
function FriendPostsFeed(props) {
  ...
  const loadMore = useCallback(function(){ setVisible(function(v){return v+20;}); }, []);
  useScrollLoad(hasMore, loadMore);

  async function openPost(feedItem) {
    setOpenedItem(feedItem);
    setPostThread(null);
  ...
  return html`<div style=${{maxWidth:680}}>
    ${shown.map(function(item, i) {
      var sender = item.sender || {};
      var msgText = (item.msgText || '').trim();
      // Wrap as a feed item for PostCard
      var feedItem = {post: item.post};
      return html`<div key=${i}>
        <div style=${{display:'flex',alignItems:'center',gap:8,padding:'8px 0 4px',
          borderTop:i>0?'1px solid #1a1a1a':'none'}}>
          <${Avatar} src=${sender.avatar} size=${22}
            onClick=${function(){props.onChannel&&props.onChannel(sender.handle);}}/>
          <span style=${{color:'#aaa',fontSize:12}}>
            <span style=${{color:'#e0e0e0',fontWeight:600,cursor:'pointer'}}
              onClick=${function(){props.onChannel&&props.onChannel(sender.handle);}}>
              ${sender.displayName||sender.handle}
            </span>
            <span style=${{marginLeft:5,color:'#555'}}>shared</span>
            ${msgText?html`<span style=${{marginLeft:5,color:'#888',fontStyle:'italic'}}>"${msgText}"</span>`:null}
            <span style=${{marginLeft:5,color:'#444',fontSize:11}}>${ago(item.sentAt)}</span>
          </span>
        </div>
        <${PostCard} item=${feedItem}
          session=${sess}
          onChannel=${props.onChannel}
          onOpenPost=${openPost}
        />
    ...
    ${hasMore?html`<div style=${{textAlign:'center',padding:16}}>
      <button onClick=${function(){setVisible(function(v){return v+20;})}}
        style=${{background:'none',border:'1px solid #333',color:'#aaa',padding:'8px 24px',cursor:'pointer',fontSize:13,borderRadius:0}}>
        Load more
      </button>
    </div>`:null}
  </div>`;
...
function FriendCard(props) {
  ...
  return html`<div style=${{cursor:'pointer'}}>
    <div style=${{display:'flex',alignItems:'center',gap:8,marginBottom:6,height:34,overflow:'hidden'}}>
      ...
      ${needsMore?html`<span onClick=${function(e){e.stopPropagation();setOpen(function(v){return !v;});}} style=${{color:'#00FF07',fontSize:11,fontWeight:600,cursor:'pointer',flexShrink:0,marginLeft:4}}>${open?'Close':'Expand'}</span>`:null}
    ...
    <div onClick=${function(){if(!open)props.onWatch&&props.onWatch(post);}}
      ...
      style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative',
        outline:thumbHov?'2px solid #00FF07':'2px solid transparent',transition:'outline 0.15s'}}>
      <div style=${{position:'absolute',top:0,left:0,right:0,bottom:0}}>
        <${Thumb} src=${thumb}/>
      </div>
...
function SubsPage(props) {
  ...
  const tabSt = function(a){ return {
    ...
    borderBottom:'3px solid '+(a?'#00FF07':'transparent')
  };};
...
function FeedPage(props) {
  ...
  const tabSt = function(active) { return {
    ...
    borderBottom:'3px solid '+(active?'#00FF07':'transparent'),cursor:'pointer'
  };};
...
function Sidebar(props) {
  ...
  return html`<aside style=${{position:'fixed',top:56,left:0,bottom:0,width:open?240:72,background:'#0f0f0f',
    ...
    ${feeds.length>0?html`
      <div style=${{height:1,background:'#00FF07',margin:'10px 0 6px'}}/>
    ...
    <form onSubmit=${submit} style=${{display:'flex',gap:10,alignItems:'flex-start'}}>
      ...
      <div style=${{flex:1}}>
        ...
        <div style=${{display:'flex',justifyContent:'flex-end',gap:10,marginTop:6,alignItems:'center'}}>
          ...
          ${done?html`<span style=${{color:'#00FF07',fontSize:12}}>Posted!</span>`:null}
          <button type='submit' disabled=${posting||!text.trim()}
            style=${{background:posting||!text.trim()?'#1a1a1a':'#00FF07',
  ...
  function ConvoRow(rp){
    ...
    return html`<div style=${{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:'1px solid #1e1e1e'}}>
      ...
      <button onClick=${function(){sendTo(c.id);}} disabled=${isSending||isSent}
        style=${{background:isSent?'#003300':isSending?'#1a1a1a':'#00FF07',color:isSent?'#00FF07':isSending?'#555':'#000',
    ...
    <div onClick=${function(e){e.stopPropagation();}}
      ...
      ${sess&&!dmErr?html`<div>
        <input value=${search} onInput=${onSearchInput} placeholder="Search for a user to send to…"
          ...
          onFocus=${function(e){e.target.style.borderColor='#00FF07';}} onBlur=${function(e){e.target.style.borderColor='#333';}}/>
        ...
        ${search.trim()&&searchRes.length?html`<div style=${{marginBottom:12,maxHeight:180,overflowY:'auto',border:'1px solid #222',background:'#111'}}>
          ${searchRes.map(function(a){return html`<div key=${a.did} style=${{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',borderBottom:'1px solid #1e1e1e'}}>
            ...
            <button onClick=${function(){sendToActor(a.did);}} disabled=${sending==='new_'+a.did||sent['new_'+a.did]}
              style=${{background:sent['new_'+a.did]?'#003300':'#00FF07',color:sent['new_'+a.did]?'#00FF07':'#000',border:'none',padding:'6px 12px',fontSize:12,fontWeight:600,cursor:'pointer',borderRadius:0}}>
    ...
    <div style=${{flex:1,minWidth:0}}>
      ...
      <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12,marginBottom:16}}>
        ...
        <div style=${{display:'flex',gap:8,flexWrap:'wrap'}}>
          ...
          <button onClick=${toggleRepost} style=${bSt(reposted,'#003300')}><svg width="16" height="16" viewBox="0 0 24 24" fill=${reposted?'#00FF07':'currentColor'}><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/></svg>${fmt(repostCount)}</button>
...
function EditProfileModal(props) {
  ...
  const fileLabelSt = function(chosen) { return {
    ...
    border:'2px dashed '+(chosen?'#00FF07':'#3f3f3f'),cursor:'pointer',
    background:chosen?'rgba(0,255,7,0.05)':'#111',transition:'border-color 0.15s'
  };};
    ...
    <div onClick=${function(e){e.stopPropagation();}}
      ...
      <div style=${{marginBottom:20}}>
        ...
        <label style=${fileLabelSt(!!bannerFile)}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill=${bannerFile?'#00FF07':'#555'}><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg>
      ...
      <div style=${{marginBottom:20}}>
        ...
        <div style=${{display:'flex',alignItems:'center',gap:16,marginBottom:8}}>
          ...
          <label style=${Object.assign({},fileLabelSt(!!avatarFile),{flex:1})}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill=${avatarFile?'#00FF07':'#555'}><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
      ...
      <button onClick=${save} disabled=${saving}
        style=${{width:'100%',padding:14,background:saving?'#003300':'#00FF07',color:saving?'#00FF07':'#000',
          border:'none',fontSize:15,fontWeight:700,borderRadius:0,
...
/>`;})}
      ...
      <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:14}} onClick=${function(e){e.stopPropagation();}}>
        <div style=${{display:'flex',gap:8,flexWrap:'wrap'}}>
          <button onClick=${toggleLike} style=${{background:liked?'rgba(0,255,7,0.12)':'#1a1a1a',border:'1px solid '+(liked?'#00FF07':'#333'),color:liked?'#00FF07':'#aaa',cursor:sess?'pointer':'default',display:'flex',alignItems:'center',gap:6,fontSize:15,padding:'10px 16px',borderRadius:0}}><svg width="18" height="18" viewBox="0 0 24 24" fill=${liked?'#00FF07':'currentColor'}><path d="M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2z"/></svg><span>${fmt(likeCount)}</span></button>
          ...
          <button onClick=${toggleRepost} style=${{background:reposted?'rgba(0,255,7,0.12)':'#1a1a1a',border:'1px solid '+(reposted?'#00FF07':'#333'),color:reposted?'#00FF07':'#aaa',cursor:sess?'pointer':'default',display:'flex',alignItems:'center',gap:6,fontSize:15,padding:'10px 16px',borderRadius:0}}><svg width="18" height="18" viewBox="0 0 24 24" fill=${reposted?'#00FF07':'currentColor'}><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/></svg><span>${fmt(repostCount)}</span></button>
  ...
  return html`<div>
    ${!props.hideFilter?html`<div style=${{display:'flex',gap:8,marginBottom:16}}>
      <button onClick=${function(){setFilterOwn(false);setVisible(10);}}
        style=${{padding:'6px 14px',border:'none',background:!filterOwn?'#00FF07':'#1a1a1a',
...
// ── ChannelDMsTab — shared posts/videos between user and this channel ─────────
function ChannelDMsTab(props) {
  ...
  useEffect(function(){
    ...
    async function load(){
      ...
      try{
        // Get or find the convo between user and this channel
        var cr=await api(chatUrl('/chat.bsky.convo.getConvoForMembers')+'&members[]='+encodeURIComponent(sess.did)+'&members[]='+encodeURIComponent(channelDid),
          {headers:{Authorization:'Bearer '+sess.accessJwt}});
        if(!cr.ok){
          if(cr.status===404||cr.status===400){if(!cancelled){setLoading(false);setErr('no_convo');} return;}
          if(cr.status===401||cr.status===403){if(!cancelled){setLoading(false);setErr('DM access denied — check your App Password has Direct Messages enabled.');} return;}
          throw new Error(await cr.text());
        }
        var cd=await cr.json();
        var cid=cd.convo&&cd.convo.id;
        if(!cancelled) setConvoId(cid);
        // Get messages
        var mr=await api(chatUrl('/chat.bsky.convo.getMessages')+'&convoId='+encodeURIComponent(cid)+'&limit=100',
          {headers:{Authorization:'Bearer '+sess.accessJwt}});
        if(!mr.ok) throw new Error(await mr.text());
        var _raw=await mr.json(); var allMsgs=_raw.messages||[]; console.log('[DMs] total msgs:', allMsgs.length, JSON.stringify(allMsgs.slice(0,2)));
        // Keep only messages that have embeds or text, sort oldest first for chat feel
        // Show all non-deleted messages
        // Keep all non-deleted messages — text can be '' (empty string with embed)
        var withContent=allMsgs.filter(function(m){
          var t=m['$type']||'';
          return t!=='chat.bsky.convo.defs#deletedMessageView';
        });
        console.log('[DMs] total:', allMsgs.length, 'withContent:', withContent.length,
          'sample:', JSON.stringify(allMsgs[0]||{}));
        withContent.reverse();
        // Hydrate any embedded post URIs
        var hydrated={};
  ...
  if(err==='no_convo') return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>
    ...
    <div>No DM history with this channel.</div>
    <div style=${{fontSize:12,marginTop:8,color:'#555'}}>Use the Share button on a video to start a conversation.</div>
  </div>`;
  if(!msgs.length&&!loading) return html`<div style=${{padding:'32px 0',textAlign:'center',color:'#aaa'}}>
    <div>No messages found.</div>
    <div style=${{fontSize:12,marginTop:8,color:'#555'}}>If you have DMs with this account, check the browser console (F12) for [DMs] logs.</div>
  </div>`;
  if(err) return html`<div style=${{padding:'32px 0',color:'#ff6666',fontSize:14}}>${err}</div>`;

  return html`<div style=${{maxWidth:680,margin:'0 auto',display:'flex',flexDirection:'column',gap:0,padding:'16px 0'}}>
    ${msgs.map(function(m,i){
      var isMine=m._isMine;
      var post=m._post;
      var hasEmbed=!!(m.embed);
      var embedUri=m.embed&&m.embed.record&&m.embed.record.uri;
      ...
      return html`<div key=${i} style=${{display:'flex',flexDirection:'column',
        alignItems:isMine?'flex-end':'flex-start',padding:'6px 16px'}}>
        <div style=${{fontSize:11,color:'#555',marginBottom:3}}>${isMine?'You':'Channel'} · ${ago(m.sentAt)}</div>
        ${hasText?html`<div style=${{background:isMine?'rgba(0,255,7,0.1)':'#1e1e1e',color:'#f1f1f1',
          padding:'10px 14px',fontSize:14,lineHeight:1.55,maxWidth:'80%',
          border:'1px solid '+(isMine?'#00FF07':'#2a2a2a')}}>
          ${m.text}
        </div>`:null}
        ${hasEmbed?html`<div style=${{marginTop:hasText?8:0,width:'100%',maxWidth:460}}>
          ${post?(isVideo
            ? html`<${VideoCard} post=${post} onWatch=${props.onWatch} onChannel=${props.onChannel}/>`
            : html`<div style=${{background:'#141414',border:'1px solid #222',padding:16,cursor:'pointer'}}
                ...
                <div style=${{display:'flex',gap:10,alignItems:'center',marginBottom:8}}>
                  ...
                  <div style=${{color:'#f1f1f1',fontSize:13,fontWeight:600}}>${post.author&&(post.author.displayName||post.author.handle)}</div>
                </div>
                ${(post.record&&post.record.text)?html`<div style=${{fontSize:14,color:'#ddd',lineHeight:1.5,whiteSpace:'pre-wrap'}}>${post.record.text}</div>`:null}
              </div>`)
            : embedUri?html`<a href=${'https://bsky.app/profile/'+embedUri.split('/')[2]+'/post/'+embedUri.split('/').pop()}
                target="_blank" rel="noreferrer"
                style=${{display:'block',background:'#1a1a1a',border:'1px solid #333',padding:'10px 14px',
                  color:'#00FF07',fontSize:13,wordBreak:'break-all',textDecoration:'none'}}
                onMouseEnter=${function(e){e.currentTarget.style.borderColor='#00FF07';}}
                onMouseLeave=${function(e){e.currentTarget.style.borderColor='#333';}}>
                🔗 ${embedUri}
              </a>`:null
          }
        </div>`:null}
...
function ChannelPage(props) {
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #00FF07'}}>
      <div style=${{display:'flex',alignItems:'center',gap:20,padding:'20px 0 20px'}}>
        ...
        <div style=${{flex:1,minWidth:0}}>
          <h1 style=${{color:'#f1f1f1',fontSize:24,fontWeight:700,marginBottom:4,display:'flex',alignItems:'center',gap:10}}>
            ...
            ${!isOwn&&sess&&d.viewer&&d.viewer.following&&d.viewer.followedBy?html`<span style=${{fontSize:13,fontWeight:600,color:'#00FF07',border:'1px solid #00FF07',padding:'2px 8px',letterSpacing:0.5}}>Friends</span>`:null}
          </h1>
        ...
        ${isOwn
          ? html`<button onClick=${function(){setShowEdit(true);}}
              style=${{background:'#1a1a1a',border:'1px solid #00FF07',color:'#00FF07',padding:'10px 20px',
                fontWeight:600,fontSize:14,cursor:'pointer',borderRadius:0,transition:'background 0.15s'}}
              onMouseEnter=${function(e){e.currentTarget.style.background='rgba(0,255,7,0.1)';}}
      ...
      <div style=${{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div style=${{display:'flex'}}>
          ${(['Content','Liked'].concat(sess&&!isOwn?['DMs']:[])).map(function(t){return html`<button key=${t} onClick=${function(){openTab(t);}}
            style=${{padding:'12px 20px',background:'none',border:'none',color:tab===t?'#f1f1f1':'#aaa',
              fontSize:14,fontWeight:tab===t?500:400,borderBottom:'3px solid '+(tab===t?'#00FF07':'transparent'),cursor:'pointer'}}>${t}</button>`;
          })}
        </div>
        ${tab==='Content'?html`<div style=${{display:'flex',gap:4,marginRight:8}}>
          <button onClick=${function(){openContentSub('Videos');}} style=${{padding:'6px 14px',background:contentSub==='Videos'?'#00FF07':'none',color:contentSub==='Videos'?'#000':'#aaa',border:'none',fontSize:13,fontWeight:600,cursor:'pointer',borderRadius:0}}>Videos</button>
          <button onClick=${function(){openContentSub('Posts');}}  style=${{padding:'6px 14px',background:contentSub==='Posts'?'#00FF07':'none',color:contentSub==='Posts'?'#000':'#aaa',border:'none',fontSize:13,fontWeight:600,cursor:'pointer',borderRadius:0}}>Posts</button>
        </div>`:null}
      </div>
    </div>
...
function App() {
  ...
  const handleChannel = useCallback(async function(actor){
    ...
    try{
      ...
      // Paginate with a hard cap to prevent infinite loops
      const seen = new Set();
      const vids = [];
      let cursor = null;
      let pageCount = 0;
