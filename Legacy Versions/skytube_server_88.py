...
function applyBg(url){
  var el=document.getElementById('raccnet-bg');
  if(!el){
    el=document.createElement('div');
    el.id='raccnet-bg';
    el.style.cssText='position:fixed;top:0;left:0;right:0;bottom:0;z-index:-1;pointer-events:none;background-size:cover;background-position:center;background-attachment:fixed;';
    document.body.appendChild(el);
  }
  el.style.backgroundImage=url?'url('+url+')':'none';
  el.style.opacity=url?'0.25':'0'; // subtle so UI stays readable
}
