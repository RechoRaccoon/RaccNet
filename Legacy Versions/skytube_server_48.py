...
function VideoCard(props) {
  ...
  return html`<div style=${{cursor:'pointer',outline:cardHov?'2px solid #00FF07':'2px solid transparent',transition:'outline 0.15s'}}
    onClick=${function(){props.onWatch(post);}}
...
function ShareModal(props) {
  ...
  useEffect(function(){
    if(!sess) return;
    api(CHAT_PROXY+'/chat.bsky.convo.listConvos',{headers:{Authorization:'Bearer '+sess.accessJwt}})
      .then(function(r){
        if(r.status===401||r.status===403){setDmErr(true);setLoading(false);return null;}
      ...
      .then(function(d){
        ...
        setLoading(false);
      }).catch(function(){setLoading(false);setErr('Failed to load DMs');});
  },[]);
  ...
  async function sendTo(convoId){
    ...
    try{
      var text=(msg.trim()?msg.trim()+'\n\n':'')+postUrl;
      var r=await api(CHAT_PROXY+'/chat.bsky.convo.sendMessage',{
        method:'POST',
        headers:{Authorization:'Bearer '+sess.accessJwt,'Content-Type':'application/json'},
        body:JSON.stringify({convoId:convoId,message:{text:text}})
      });
  ...
  async function sendToActor(actorDid){
    ...
    try{
      var cr=await api(CHAT_PROXY+'/chat.bsky.convo.getConvoForMembers?members[]='+encodeURIComponent(sess.did)+'&members[]='+encodeURIComponent(actorDid),
        {headers:{Authorization:'Bearer '+sess.accessJwt}});
  ...
  return html`<div onClick=${props.onClose}
    ...
    <div onClick=${function(e){e.stopPropagation();}}
      ...
      ${dmErr?html`<div style=${{background:'#1a0a00',border:'1px solid #884400',padding:'10px 14px',color:'#ffaa66',fontSize:13,marginBottom:16,lineHeight:1.5}}>
        ⚠️ DM access denied. Your App Password may not have Direct Messages enabled.
        Create a new App Password with the "Direct Messages" option checked in Bluesky Settings → Privacy & Security → App Passwords.
      </div>`:null}
      ${sess&&!dmErr?html`<div>
...
function LikedTab(props) {
  ...
  const tabSt = function(a){ return {
    ...
    color:a?'#f1f1f1':'#aaa',fontWeight:a?600:400,
    borderBottom:'2px solid '+(a?'#00FF07':'transparent')
  };};
  return html`<div>
    <div style=${{display:'flex',gap:0,marginBottom:20,borderBottom:'1px solid #1e1e1e'}}>
      <button style=${tabSt(sub==='Videos')} onClick=${function(){setSub('Videos');}}>Videos</button>
      <button style=${tabSt(sub==='Posts')}  onClick=${function(){setSub('Posts');}}>Posts</button>
    </div>
    ${sub==='Videos'?html`<${VideoGrid} videos=${props.videos||[]} loading=${false} onWatch=${props.onWatch} onChannel=${props.onChannel}/>`:null}
    ${sub==='Posts'?html`<${ChannelPostsFeed} posts=${props.posts||[]} loading=${false} session=${props.session} onChannel=${props.onChannel} onWatch=${props.onWatch}/>`:null}
  </div>`;
}
...
function ChannelPage(props) {
  ...
  if (incomingDid && incomingDid !== prevPropsDidRef.current) {
    prevPropsDidRef.current = incomingDid;
    // Reset overrides so the new channel's data shows correctly
  ...
  async function loadLikes(did) {
    ...
    try {
      var sess2 = props.session;
      var opts  = sess2 ? {headers:{Authorization:'Bearer '+sess2.accessJwt}} : {};
      var url   = AUTH_PROXY+'/app.bsky.feed.getActorLikes?actor='+encodeURIComponent(did)+'&limit=100';
      var r     = await api(url, opts);
      if(r.ok){
        var data  = await r.json();
        var items = data.feed||[];
        var posts = items.map(function(i){return i;});
        var vids  = items.map(function(i){return i.post;}).filter(function(p){return isVid(p)||isVidRaw(p);});
        setLikedPosts(posts);
        setLikedVids(vids);
        setLikedLoaded(true);
      }
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #272727'}}>
      ...
      <div style=${{display:'flex'}}>
        ${(isOwn?['Videos','Posts','Liked']:['Videos','Posts']).map(function(t){return html`<button key=${t} onClick=${function(){openTab(t);}}
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path in ("/", "/index.html"):
            ...
            self._proxy("GET", "https://bsky.social/" + self.path[len("/proxy/chat/"):], extra_headers={"atproto-proxy": "did:web:api.bsky.chat#bsky_chat"})
    ...
    def do_POST(self):
        if self.path.startswith("/proxy/auth/"):
            ...
            length = int(self.headers.get("Content-Length", 0))
            body   = self.rfile.read(length) if length else b""
            self._proxy("POST", "https://bsky.social/" + self.path[len("/proxy/chat/"):], body=body, extra_headers={"atproto-proxy": "did:web:api.bsky.chat#bsky_chat"})
        elif self.path.startswith("/proxy/video/"):
            length = int(self.headers.get("Content-Length", 0))
            body   = self.rfile.read(length) if length else b""
