...
function UploadModal(props) {
  ...
  async function upload() {
    ...
    try {
      ...
      // video.bsky.app requires its own short-lived token, NOT the regular accessJwt
      setStatus('uploading');
      setProgress('Authenticating with video service…');
      const saRes = await fetch(
        AUTH_PROXY+'/com.atproto.server.getServiceAuth?aud=did:web:video.bsky.app&lxm=app.bsky.video.uploadVideo',
