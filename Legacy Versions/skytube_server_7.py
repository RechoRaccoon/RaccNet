...
function UploadModal(props) {
  ...
  async function upload() {
    ...
    try {
      ...
      // ── Step 2: Upload the (possibly stitched) video to video.bsky.app ──────
      setStatus('uploading');
      ...
      const vRes = await fetch(
        ...
        { method:'POST', headers:{'Content-Type': finalVideoFile.type||'video/mp4', 'Authorization':'Bearer '+sess.accessJwt}, body: finalVideoFile }
      );
      ...
      // ── Step 3: Poll until Bluesky finishes processing ───────────────────────
  ...
  const STEPS = [
    ...
    {id:'uploading',  label:'Uploading video to Bluesky'},
    ...
    <div onClick=${function(e){e.stopPropagation();}}
      ...
      `:html`
        ...
        <button onClick=${upload} disabled=${busy||!videoFile||!title.trim()}
          style=${{width:'100%',padding:14,background:busy?'#3a0000':'#ff0000',color:'#fff',
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _process_video(self):
        """
        Receives a multipart/form-data POST with two fields:
          - video:     the original video file
          - thumbnail: the image to prepend as the first frame

        Uses FFmpeg to create a version of the video that starts with
        one frame (1/30 s) of the thumbnail image, then continues with
        the original video.  Returns the stitched video as bytes.
        """
        tmpdir = tempfile.mkdtemp(prefix='skytube_')
        try:
            ...
            # Simple multipart parser (avoids cgi module deprecation issues)
            ...
            for part in parts:
                ...
                try:
                    header_section, _, body_data = part.partition(b'
...
--')
                disp_match = re.search(rb'name="([^"]+)"', header_section)
                if disp_match:
                    fields[disp_match.group(1).decode()] = body_data

            if 'video' not in fields:
                self._json_error(400, 'Missing video field')
                return
            if 'thumbnail' not in fields:
                self._json_error(400, 'Missing thumbnail field')
                return
            ...
            if not shutil.which('ffmpeg'):
                self._json_error(500, 'FFmpeg is not installed. Run: winget install ffmpeg  (then restart the server)')
                return

            # ── Get video dimensions so thumbnail matches ──────────────────────
            ...
            if probe.returncode == 0 and probe.stdout.strip():
                try:
                    parts_dim = probe.stdout.strip().split(',')
                    w, h = int(parts_dim[0]), int(parts_dim[1])
                except Exception:
                    pass

            # ── FFmpeg: prepend thumbnail as first frame (1/30 s) ─────────────
            # Strategy:
            #   1. Scale thumbnail to match video resolution
            #   2. Create a 1-frame clip (duration = 1/30 s) from the thumbnail
            #   3. Concatenate: [thumb_clip] + [original_video]
            #   4. Copy audio from original video
            cmd = [
                'ffmpeg', '-y',
                # Input 0: thumbnail image (loop for 1/30 second)
                '-loop', '1', '-t', '0.0334', '-i', thumb_path,
                # Input 1: original video
                '-i', video_path,
                # Filter: scale thumb to video size, then concat
                '-filter_complex',
                '[0:v]scale={w}:{h},setsar=1,fps=30[thumb];'
                '[thumb][1:v]concat=n=2:v=1:a=0[v]'.format(w=w, h=h),
                '-map', '[v]',
                # Copy audio from original video (stream 1), if it exists
                '-map', '1:a?',
                '-c:v', 'libx264',
                '-preset', 'fast',
                '-crf', '23',
            ...
            if result.returncode != 0:
                ...
                self._json_error(500, 'FFmpeg error: ' + err_msg)
                return

            # ── Send the stitched video back ───────────────────────────────────
            ...
            self._json_error(500, 'FFmpeg timed out processing the video')
