      ...
      <div ref=${scrollRef}
        ...
        ${(props.actors||[]).map(function(actor,i){
          return html`<div key=${actor.did||i}
            ...
            onMouseEnter=${function(e){e.currentTarget.querySelector('div').style.border='4px solid #00FF07';}}
            onMouseLeave=${function(e){e.currentTarget.querySelector('div').style.border=actor.hasNewVideo?'4px solid #00FF07':'4px solid #2a2a2a';}}>
            <div style=${{width:110,height:110,borderRadius:'50%',overflow:'hidden',background:'#272727',
              border:actor.hasNewVideo?'4px solid #00FF07':'4px solid #2a2a2a',
    ...
    async function load(){
      ...
      try{
        ...
        if(found.length>0&&!cancelled){
          ...
          // Build unique sender list in order of most recent message
          ...
          if(!cancelled){setItems(videoItems);setSenders(senders);}
        }
  ...
  return html`<div style=${{padding:24}}>
    ...
    <div style=${{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:24,alignItems:'start'}}>
      ${items.map(function(item,i){
        ...
        var maxLen=80;
        var truncated=msgText.length>maxLen?msgText.slice(0,maxLen)+'…':msgText;
...
function FriendCard(props) {
  ...
  const display=expanded?msgText:(msgText.slice(0,MAX)+(needsMore?'...':''));
  const author=post.author||{};
  const rec=post.record||{};
  ...
  return html`<div style=${{background:'#141414',border:'1px solid #1e1e1e',display:'flex',flexDirection:'column'}}>
    <div style=${{padding:'10px 12px 6px',display:'flex',alignItems:'center',gap:8,height:46,boxSizing:'border-box'}}>
      ...
      <div style=${{minWidth:0,flex:1,overflow:'hidden'}}>
        <span style=${{color:'#f1f1f1',fontSize:12,fontWeight:600}}>${sender.displayName||sender.handle||'Unknown'}</span>
        ${msgText?html`<span style=${{color:'#666',fontSize:11,marginLeft:6}}>${display}</span>`:html`<span style=${{color:'#555',fontSize:11,marginLeft:6,fontStyle:'italic'}}>shared a video</span>`}
        ${needsMore?html`<span onClick=${function(e){e.stopPropagation();setExpanded(function(v){return !v;});}} style=${{color:'#00FF07',fontSize:11,cursor:'pointer',marginLeft:4,flexShrink:0}}>${expanded?'less':'more'}</span>`:null}
      </div>
      <div style=${{color:'#444',fontSize:10,flexShrink:0}}>${ago(item.sentAt)}</div>
    </div>
    <div onClick=${function(){props.onWatch&&props.onWatch(post);}} style=${{cursor:'pointer'}}
      onMouseEnter=${function(){setThumbHov(true);}} onMouseLeave=${function(){setThumbHov(false);}}>
      <div style=${{width:'100%',paddingBottom:'56.25%',overflow:'hidden',background:'#1a1a1a',position:'relative',
        outline:thumbHov?'2px solid #00FF07':'2px solid transparent',transition:'outline 0.15s'}}>
        <div style=${{position:'absolute',top:0,left:0,right:0,bottom:0}}><${Thumb} src=${thumb}/></div>
      </div>
    </div>
    <div style=${{display:'flex',gap:10,padding:'8px 10px',alignItems:'flex-start',cursor:'pointer',height:68,boxSizing:'border-box'}} onClick=${function(){props.onWatch&&props.onWatch(post);}}>
      <${Avatar} src=${author.avatar} size=${32}
        onClick=${function(e){e.stopPropagation();props.onChannel&&props.onChannel(author.handle);}}/>
      <div style=${{flex:1,minWidth:0}}>
        <div class="clamp2" style=${{fontSize:13,fontWeight:500,color:'#f1f1f1',lineHeight:1.3}}>${title}</div>
        <div style=${{fontSize:12,color:'#aaa',marginTop:2}}>${author.displayName||author.handle}</div>
        <div style=${{fontSize:11,color:'#555'}}>${fmt(post.likeCount||0)} likes</div>
      </div>
    </div>
  </div>`;
}
...
function ChannelPage(props) {
  ...
  async function openTab(t) {
    setTab(t);
    if (t === 'Posts' && d && d.did && d.did !== loadedDid) {
      await loadPosts(d.did);
    }
    if (t === 'Liked' && !likedLoaded && d && d.did) {
      await loadLikes(d.did);
    }
    // DMs tab just mounts ChannelDMsTab which handles its own loading
  }
  ...
  return html`<div>
    ...
    <div style=${{padding:'0 24px',borderBottom:'1px solid #00FF07'}}>
      ...
      <div style=${{display:'flex'}}>
        ${(['Videos','Posts','Liked'].concat(sess&&!isOwn?['DMs']:[])).map(function(t){return html`<button key=${t} onClick=${function(){openTab(t);}}
    ...
    <div style=${{padding:24}}>
      ...
      ${tab==='Posts'?html`<${ChannelPostsFeed} posts=${allPosts} loading=${postsLoading} session=${props.session} onChannel=${props.onChannel} onWatch=${props.onWatch}/>`:null}
