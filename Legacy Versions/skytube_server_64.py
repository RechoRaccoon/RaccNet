...
function FriendCard(props) {
  ...
  return html`<div style=${{background:'#141414',border:'1px solid #1e1e1e',display:'flex',flexDirection:'column'}}>
    ...
    <div style=${{padding:'0 8px 8px'}}>
      <${VideoCard} post=${post} onWatch=${props.onWatch} onChannel=${props.onChannel}/>
    </div>
  </div>`;
}
...
function SubsPage(props) {
  ...
  const tabSt = function(a){ return {
    ...
    borderBottom:'3px solid '+(a?'#f1f1f1':'transparent')
  };};
  ...
  useEffect(function(){
    ...
    async function load(){
      ...
      try{
        ...
        var withContent=allMsgs.filter(function(m){return (m.text&&m.text.trim())||m.embed;}); console.log('[DMs] withContent:', withContent.length);
        withContent.reverse();
