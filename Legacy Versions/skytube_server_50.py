...
function ShareModal(props) {
  ...
  // Get a service auth token scoped to the chat service
  async function getChatToken() {
    var saR = await api(AUTH_PROXY+'/com.atproto.server.getServiceAuth?aud=did:web:api.bsky.chat&lxm=chat.bsky.convo.sendMessage',
      {headers:{Authorization:'Bearer '+sess.accessJwt}});
    if(!saR.ok) {
      // Try without lxm restriction
      saR = await api(AUTH_PROXY+'/com.atproto.server.getServiceAuth?aud=did:web:api.bsky.chat',
        {headers:{Authorization:'Bearer '+sess.accessJwt}});
    }
    if(!saR.ok) throw new Error('Could not get chat token');
    return (await saR.json()).token;
  }
  ...
  useEffect(function(){
    ...
    (async function(){
      try{
        ...
        var r=await api(CHAT_PROXY+'/chat.bsky.convo.listConvos',{headers:{Authorization:'Bearer '+token}});
...
function LikedTab(props) {
  const [sub, setSub] = useState('Liked Videos');
  ...
  return html`<div>
    <div style=${{display:'flex',justifyContent:'flex-end',borderBottom:'1px solid #272727',marginBottom:20}}>
      <button style=${tabSt(sub==='Liked Videos')} onClick=${function(){setSub('Liked Videos');}}>Liked Videos</button>
      <button style=${tabSt(sub==='Liked Posts')}  onClick=${function(){setSub('Liked Posts');}}>Liked Posts</button>
    </div>
